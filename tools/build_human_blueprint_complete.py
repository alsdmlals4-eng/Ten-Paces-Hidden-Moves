...
def main():
    ...
    candidates=read('data/run/vertical_slice_opponents.json')['candidates']
    ...
    b.photo(ART/arts['yang_family_spear'][2],36,135,660,565)
    b.p('상대의 수를 읽고,\n나의 무공으로 답한다.',738,647,425,30,bold=True)
    b.p('1대1 무협 전술 게임\n10칸의 일자 전장 · 열 번의 비무\n3수 → 해결 → 3수 → 해결 → 4수 → 해결',738,526,420,18)
    b.p('화려한 진검승부를 감상하면서도, 왜 합에서 이겼고 어디서 흐름이 끊겼는지 이해할 수 있는 게임을 만든다.',738,405,415,17)
    b.p('이 책은 현재 기획의 통합 편집판이다. 새 원화의 최종 채택과 게임 적용, 미구현 기능, 사람 체감 검증은 구분해 표시한다. 문서 완성과 게임 개발 완료는 같은 상태가 아니다.',738,282,415,13)
    b.note('표지: 양가창결 회마창 손 교정본 · 신규 도감 삽화. 실제 게임 화면이 아니다.',100)

    b.page('읽는 순서와 문서의 경계','같은 규칙을 여러 곳에 복제하지 않고, 역할별 책임 페이지로 나눴다.')
    sections=[('1부 · 게임 기획','핵심 재미와 경험 / 전체 플로우 / 화면 아틀라스'),('2부 · 강호행로','비전투 선택 / 사건 / 휴식 / 조사·정탐 / 수련'),('3부 · 전투 시스템','준비와 계획 / 거리·관찰 / 합·회피·방어 / 절초·결과'),('4부 · 강호의 상대','15명 전용 초상 / 10전 완성형 / 각 인물의 10→1전 표'),('5부 · 무공과 데이터','10권 × 기술·절초 3종 / 5·9성 강화 / 기본 행동 / 제작 예산'),('6부 · 구현 이해와 체크','기획→상세→자산→구현→검증 / 한글 연결 지도 / 검수와 출처')]
    ...
    b.photo(ART/'plum-star7-candidate.png',36,125,520,575)
    for i,(t,s) in enumerate([('읽어냈다','지나간 행동과 공개 거리를 보고 다음 수를 예상한다. 상대의 숨은 계획을 보거나 정답 확률을 읽는 게임이 아니다.'),('내 무공으로 풀었다','성장은 공격 숫자만 올리는 것이 아니라 거리·방어·반격·연계 선택지를 넓힌다.'),('한 수가 장면으로 남는다','예비 동작, 칼끝 궤적, 상단 충돌, 서로 다른 반동과 복귀가 판정을 설명한다.')]):
        b.panel(t,s,595,685-i*190,569,165)
    ...
    b.flow([('메인','새 여정 / 이어하기'),('시작 무공','시작 후보 6권 중 4권'),('브리핑','상대·보유 무공·제약'),('전투 준비','거리·관찰·기술 배치'),('전투 실행','3수 / 3수 / 4수 해결')],y=500,h=125)
    b.flow([('승리·보상','수련 또는 무공 전수'),('강호행로','3후보 중 1개 × 4회'),('다음 비무','2전부터 10전까지')],y=285,x=180,w=840,h=115)
    ...
    b.picture_page('화면 아틀라스 · 한눈에 보는 여정','기존의 화면 역할과 시각 밀도를 보존한 참고 지도',atlas,'시각 목표 참고. 그림 안의 이름·숫자는 이번 도감의 수치 정본이 아니다. 다음 두 페이지에서 현행 역할을 정리한다.')
    ...
    b.photo(bg,36,295,1128,390)
    b.box(58,594,370,67,INK);b.p('강호행로  ·  두 번째 선택 / 네 번',72,637,340,17,PAPER,True)
    ...
    b.page('4부 · 강호의 상대들','15명의 인물 후보, 한 회차 10전. 초상은 신규 제작·검수한 도감용 원화다.','새 원화 최종 채택·게임 연결은 별도')
    for i,p in enumerate(people):
        xx=36+(i%5)*228;yy=80+(2-i//5)*202
        b.photo(ROOT/p['portrait'],xx,yy+29,212,171)
        b.p(p['name']+' · '+p['manual'],xx+5,yy+24,208,11,bold=True)
    ...
    b.table(['항목','이번 상세 권장안','보호 기준'],[
        ...
        ['주력 무공 성수','3·3·5·5·7·7·9·9·10·10성','보조 무공을 임의 혼합하지 않음'],
    ...
    b.note('150행은 인물별 등장 설계표다. 현재 게임이 이 수치를 사용한다는 뜻이 아니다. 사람 밸런스·추첨 분포·저장 호환 검증은 6부에 남겼다.',122)

    for p in people:
        c=bycandidate[p['id']];m=bymanual[c['signature_manual_id']];top=p['stages'][0]
        b.page(p['name']+' · '+top['epithet'],'10전 최고 성장 기준  /  '+p['identity'],'신규 초상 · 수치 상세 권장안 / 게임 연결 전')
        ...
        b.p('보유 무공  '+p['manual']+' · 10성',505,478,650,20,BLUE,True)
        b.p('기술: '+' · '.join(top['techniques'])+'\n강화: '+' · '.join(top['upgrades']),505,435,650,15)
        ...
        b.page(p['name']+' · 단계별 등장표','10전 → 1전. 보유 무공: '+p['manual']+' 1권. 자원 상한은 전 단계 체력30·기력5·내력4.','단계별 수치 권장안 / 런타임·사람 밸런스 검증 전')
        rows=[]
        for r in p['stages']:
            ...
            rows.append([str(r['stage'])+'전',r['epithet'],' / '.join(map(str,r['stats'])),str(r['stat_total']),str(r['mastery'])+'성',scope,upgrade])
        b.table(['비무','등장 별호','외공 / 근골 / 신법 / 내공 / 심안','합계','주력 무공','해금 범위','강화'],rows,[60,185,300,65,100,310,120],size=11.5)
        b.p('기술 1: '+m['cards']['star3']['name']+'  /  기술 2: '+m['cards']['star7']['name']+'  /  절초: '+m['cards']['star10']['name'],36,171,1128,13,bold=True)
        b.note('해금된 기술도 기세·기력·내력·거리·상태 조건을 충족해야 사용한다. 9전에서 절초를 먼저 경험하고 10전에서는 최고 능력으로 만난다.',125)
    ...
    checklist=[
        ...
        ['상대15명 / 여성 포함','확정 방향','인물별 정리','새 초상15','구형 이미지 소비','신규 정본 채택·연결 전'],
        ['10전별 수치·성수·별호','확정 방향','150행 권장안','도감 있음','미연결','밸런스·저장 검증 전'],
    ...
    b.panel('프로젝트 기준','현재 프로젝트 규칙·활성 문맥·승인 결정, 전투 규칙과 UI·연출 명세, 기본 행동10종·무공10권·상대15명 데이터, 행로·보상·저장 소비자. 상세 출처 해시는 문서와 함께 보존한 검증 기록에 남긴다.',36,689,550,226)
