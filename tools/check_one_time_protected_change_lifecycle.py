...
def _changed_paths(root: Path, base_sha: str) -> set[str]:
    result = _git(root, "diff", "--name-only", f"{base_sha}...HEAD")
...
def validate(root: Path, base_sha: str) -> list[str]:
    changes = _changed_paths(root, base_sha)
    return lifecycle_errors(
        ...
        archive_record_changed=any(path.endswith(ARCHIVE_RECORD_SUFFIX) for path in changes),
        adapter_changed=ADAPTER_PATH in changes,
