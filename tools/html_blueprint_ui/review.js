...
function auditGallery(unfiltered=false){
 ...
 const list=D.assets.filter(a=>(!a.details?.retired||auditFilter==='discarded')&&(unfiltered||match(a))&&(auditFilter==='all'||a.audit.flags.includes(auditFilter))&&(auditGroup==='all'||a.group.id===auditGroup));
