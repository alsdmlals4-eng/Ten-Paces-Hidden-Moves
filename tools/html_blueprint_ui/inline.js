...
function numberedBlock(b){const replacement=(D.image_replacements||[]).find(r=>r.path===b.path);if(replacement&&b.kind==='image'){const a=D.assets.find(x=>x.path===b.path&&x.scope==='MAIN_SOURCE');if(a?.size)b={...b,size:a.size,region:[0,0,a.size[0],Math.round(a.size[1]*replacement.display_fraction)]};}const row=b.kind==='image'?imageByPath.get(b.path):null;const rendered=replacement?historicalBlock(b).replace(/<button[^>]*>/,'<div class="reference-surface">').replace('</button>','</div>'):historicalBlock(b);return row?`<div data-numbered-image="${E(row.key)}">${rendered}${imageCaption(row)}${userReviewPanel(recordById(row.record_id))}</div>`:historicalBlock(b);}
