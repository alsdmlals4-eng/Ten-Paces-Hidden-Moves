...
REQUIRED_REASON_CODES = {
    "PUBLIC_RANGE_MISMATCH",
    "PUBLIC_RESOURCE_OVERCOMMIT",
    "PUBLIC_ORDERING_MISMATCH",
    "MISSING_DEFENSE_RESPONSE",
    "CONDITION_PUBLICLY_IMPOSSIBLE",
    "PREDICTION_MISS",
    "COUNTER_SELECTION_MISS",
    "POSITION_READ_MISS",
}
EXPECTED_POINTS = [1, 4, 8, 12, 15, 20]
EXPECTED_REPLAY_FLAGS = {
    "same_player_plan": True,
    "same_enemy_locked_plan": True,
    "same_targets_positions_statuses": True,
    "same_rng_seed_and_consumption_order": True,
    "reference_all_core_stats": 4,
    "non_stat_maximum_bonuses_preserved": True,
}
...
def validate_contract(contract: dict[str, Any]) -> list[str]:
    ...
    constitution = derived.get("constitution", {})
    agility = derived.get("agility", {})
    internal_power = derived.get("internal_power", {})
    insight = derived.get("insight", {})
    if constitution.get("max_health_formula") != "26 + constitution":
        errors.append("DERIVED_STAT_FORMULA_CONFLICT max_health")
    if agility.get("max_stamina_formula") != "4 + floor(agility / 4)":
        errors.append("DERIVED_STAT_FORMULA_CONFLICT max_stamina")
    if internal_power.get("max_internal_formula") != "3 + floor(internal_power / 4)":
        errors.append("DERIVED_STAT_FORMULA_CONFLICT max_internal")
    if constitution.get("defense_requires_declared_action_or_effect") is not True:
        errors.append("PASSIVE_DEFENSE_RESCUE_CONFLICT declared defense required")
    if constitution.get("passive_global_damage_reduction") is not False:
        errors.append("PASSIVE_DEFENSE_RESCUE_CONFLICT passive global reduction")
    if insight.get("hidden_plan_access") is not False:
        errors.append("STRUCTURAL_SCALING_CONFLICT HIDDEN_PLAN_ACCESS")
    if insight.get("automatic_correct_counter") is not False:
        errors.append("AUTOMATIC_CORRECT_COUNTER conflict")
    ...
    if points != EXPECTED_POINTS:
        errors.append("UNCAPPED_SANITY_BAND_CONFLICT expected 1,4,8,12,15,20")
    expected = {
        "max_health": [max_health(x) for x in EXPECTED_POINTS],
        "max_stamina": [max_stamina(x) for x in EXPECTED_POINTS],
        "max_internal": [max_internal(x) for x in EXPECTED_POINTS],
    }
    ...
    replay = contract.get("counterfactual_contract", {})
    for key, expected_value in EXPECTED_REPLAY_FLAGS.items():
        if replay.get(key) != expected_value:
            errors.append(f"COUNTERFACTUAL_REPLAY_CONFLICT {key}")
    ...
    reason_codes = set(contract.get("wrong_plan_reason_codes", []))
    for missing in sorted(REQUIRED_REASON_CODES - reason_codes):
        errors.append(f"WRONG_PLAN_REASON_CONFLICT missing {missing}")
