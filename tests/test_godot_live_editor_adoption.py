...
LEGACY_PLUGIN = ROOT / "addons/godot_ai/plugin.cfg"
SMOKE_TEST = ROOT / "tests/verify_step0.gd"
ALLOWED_PATHS = {
...
def test_descriptor_is_exact_legacy_coexistence_contract() -> None:
...
def test_source_legacy_authority_and_smoke_test_are_preserved() -> None:
    project = (ROOT / "project.godot").read_text(encoding="utf-8")
    plugin = _required_text(LEGACY_PLUGIN)
    ...
    assert 'version="3.0.5"' in plugin
    assert 'script="plugin.gd"' in plugin
    assert SMOKE_TEST.is_file()
    assert "STEP0_GODOT_VERIFY_OK" in SMOKE_TEST.read_text(encoding="utf-8")


def test_adoption_document_preserves_coexistence_and_evidence_boundaries() -> None:
...
def test_workflow_uses_one_immutable_self_contained_base_pin() -> None:
