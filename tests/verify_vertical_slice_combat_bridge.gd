...
func _run() -> void:
    ...
    var approved_art = load("res://src/ui/approved_blueprint_art.gd")
    _expect_true(portrait != null and portrait.texture is AtlasTexture and portrait.texture.atlas.resource_path == approved_art.portrait_path(str(enemy.candidate_id)), "Live bridge HUD must show the actual approved opponent.")
    for candidate_id in approved_art.PORTRAITS:
        ...
        _expect_true(portrait.texture is AtlasTexture and portrait.texture.atlas.resource_path == approved_art.portrait_path(candidate_id), "Each identity must resolve without a shared masked portrait.")
