...
func _run() -> void:
    ...
    assert("소모" not in basic_snapshot.get("row_keys", []))
    assert("효과" in basic_snapshot.get("section_titles", []))
    assert("효과 상세" not in basic_snapshot.get("section_titles", []))
    assert(str(basic_snapshot.get("primary_effect", "")) == "이동 1")
    assert("출처" not in basic_snapshot.get("row_keys", []))
    ...
    assert("출처" not in martial_snapshot.get("row_keys", []))
    var approved_illustration = panel.find_child("ApprovedManualIllustration", true, false)
    assert(approved_illustration != null)
    assert(approved_illustration.texture == load("res://src/ui/approved_blueprint_art.gd").action_illustration(martial))
    assert(panel.find_child("DetailIllustration", true, false) == null)
