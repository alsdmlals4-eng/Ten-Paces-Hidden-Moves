...
const APPROVED_BACKGROUND_PATH := "res://assets/combat/ink_wuxia/background.png"
...
func _run() -> void:
	...
	var background_floor_y := board.battle_background.get_duel_floor_y(board.size)
	...
	_expect(is_equal_approx(player_foot.y, background_floor_y), "Player feet must use the frontal courtyard floor-contact reference.")
	_expect(is_equal_approx(enemy_foot.y, background_floor_y), "Enemy feet must use the frontal courtyard floor-contact reference.")
	_expect(board.player_character.get_shadow_contact_y() <= player_foot.y, "Player contact shadow must touch, not float below, its feet.")
	_expect(board.enemy_character.get_shadow_contact_y() <= enemy_foot.y, "Enemy contact shadow must touch, not float below, its feet.")
