...
func _run() -> void:
    ...
    check(shell.find_child("MainContinueButton", true, false).disabled, "corrupt v2 cannot continue")
    shell.queue_free()
    await process_frame
    ...
    check(shell.find_child("MainContinueButton", true, false).disabled, "future schema cannot continue")
