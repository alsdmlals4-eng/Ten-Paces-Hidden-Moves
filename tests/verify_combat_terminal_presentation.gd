...
func _run() -> void:
    ...
    if ultimate_index < 0:
        ...
        if str(placement.get("card_id", "")) != "ultimate_void_sword_qi" or int(placement.get("span", 0)) != 3:
            failures.append("Selecting 파공검기 must auto-reserve timings 1~3.")
        else:
            ...
            if not board.combat_progress_button.progress_enabled:
                ...
                if str(board.get_meta("presentation_state", "")) != "plan_locked":
                    failures.append("A complete terminal ultimate must visibly lock its plan before resolution.")
                board.combat_progress_button.request_progress()
