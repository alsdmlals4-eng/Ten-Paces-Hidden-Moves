...
func _run() -> void:
    ...
    if scenarios.size() >= 2:
        ...
        _expect_eq(int(report.get("schema_version", 0)), 2, "Coverage-extension report schema version must be explicit.")
        _expect_eq(str(report.get("contract_id", "")), "TEN-DEC-20260830-BALANCE-MEASUREMENT-POLICY-COVERAGE-EXTENSION-01", "Report must identify its approved coverage-extension measurement contract.")
