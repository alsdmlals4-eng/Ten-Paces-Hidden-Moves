...
class ActionCardSourceUnificationContractTests(unittest.TestCase):
    ...
    def test_common_card_renderer_keeps_summary_visible_and_rich_detail_in_panel(self) -> None:
        ...
        self.assertIn('var summary_height := maxf(28.0, summary.get_combined_minimum_size().y)', renderer)
        self.assertIn('custom_minimum_size.y = maxf(CROSS_PLATFORM_CARD_HEIGHT, ceilf(CARD_CONTENT_TOP + summary_height + CARD_BOTTOM_PADDING))', renderer)
        self.assertIn('summary.set_anchors_preset(Control.PRESET_BOTTOM_WIDE)', renderer)
        self.assertIn('summary.offset_bottom = -CARD_BOTTOM_PADDING', renderer)
        self.assertIn('resized.connect(_layout_card_content)', renderer)
        self.assertNotIn('custom_minimum_size = Vector2(0.0, 98.0)', renderer)
        self.assertIn('"예상 위력 %d"', renderer)
        self.assertIn('기력%d · 내력%d · 거리%s', renderer)
        self.assertIn('int(action_definition.get("stamina_cost", 0))', renderer)
        self.assertIn('int(action_definition.get("internal_cost", 0))', renderer)
        self.assertIn('str(action_definition.get("range_text", "-"))', renderer)
