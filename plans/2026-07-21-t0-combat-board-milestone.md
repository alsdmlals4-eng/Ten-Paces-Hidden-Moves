...
- STEP 2 캐릭터 크기·칸 배치 기준: **사용자 승인·자산 반영**
- STEP 3 전투 배경: 대기
...
- [x] 정확히 10칸과 번호 1~10 기준을 확정했다.
- [x] 플레이어 3번·상대 8번 배치를 승인했다.
- [x] 동일 스케일과 `1.4~1.6배` 높이를 승인했다.
- [x] 발 중심과 칸 중앙 앵커 일치를 승인했다.
- [x] 승인 기준 SVG 자산을 저장소에 반영했다.
- [ ] Godot 전투판 씬에서 실제 위치를 검증했다.
- [ ] 16:9·최소 해상도·색각 폴백을 수동 검증했다.

현재 정적 구조는 완료했으며 Godot·Windows 런타임 검증은 `NOT_RUN`이다.
