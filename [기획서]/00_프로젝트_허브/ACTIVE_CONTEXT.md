...
이전 서버의120분 고정 종료가 늦게 불러온 이미지/영상 실패의 재현 원인이다. 새 주소의 Chrome/IAB에서 기존MP4가 재생됨을 확인했고 활성 페이지 확인으로 수명을 유지하도록 교정한다. 닫은 세션을 영구 준비 상태로 간주하지 않고 「블루프린트 열기.cmd」가 현재 발행본을 연결한다. 기존 미확인 원화·PR342 모션을 새 승인이나 현재main 구현으로 승격하지 않는다. 실제 사용자 리뷰revision3/3항목과 원래dirty45개 보호. 현행2회 전체검토와 추가표화 집중검토·브라우저 검증을 완료했다. 실제 사용자리뷰 해시 불변/제품diff0이다. 검증 세부/정상PR 병합 현재 상태는 receipt.media_tables_followup/GitHub live에서 확인한다.
