...
PR #84·#86·#87은 main에 병합된 역사 계보다. 현재 기획 권위는 PR 메타데이터가 아니라 main의 Decision·approved contract·책임 원본이다. PR #85 HTML PoC는 `[보류]`로 닫혔으며 현재 제품 권위가 아니다.
...
현재 전투 구조는 관찰·추론·비공개 계획·복기 코어와 일치한다. 즉시 구조를 교체할 근거는 없지만 다음 위험은 미검증 상태다.

| 위험 | 상태 | 핵심 검증 |
|---|---|---|
| `RESOURCE_SATURATION_RISK` | P1·미실측 | 자원 상한률·자동 회복 낭비·명상/준비 선택률 |
| `CONDITION_CALIBRATION_RISK` | P1·미실측 | 기술별 조건 성공률·난도 범위 이탈 |
...
위험 실측 전 자원 회복·조건 가격·능력치 공식을 임의 변경하지 않는다. 사람 검증 없이 재미·밸런스 PASS를 주장하지 않는다.

## 생명주기 요약

- `[현행]`: 2026-08-04 전투 가격·repricing·기술1 조건/5성 계약.
- `[대체됨]`: 2026-08-02 구형 사거리 가격, 2026-08-03 구형 기술1 효과 Decision·contract.
...
```yaml
planning_checkpoint: MERGED_CANON_7_OF_10
