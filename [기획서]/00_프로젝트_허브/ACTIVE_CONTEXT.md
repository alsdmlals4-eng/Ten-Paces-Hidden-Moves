...
- 절초 목록은 플레이어 절초 기세 바로 아래에 planning 중 상시 표시한다. 강공 선택과 무관하게 기세 5·빈 연속 수·입력 잠금 상태가 선택 가능 여부를 결정한다.
...
절초 3종·피격 중단·강건·밀착·전투 불능 결과 상태는 구현 및 자동 검증 완료다. `[집중]`은 제거했다. 피해 중단은 그 수의 아직 실행 전 행동에만 적용하고 이후 수 계획과 같은 수 합은 유지한다. 2·3슬롯 행동은 앞 점유 수를 `[준비]`, 마지막 점유 수를 `[실행]`으로 확정 이벤트와 로그에 남긴다. 태세 성공 뒤 `[강건]`은 상태 패널의 금빛 태그·설명으로 동기화되고 소비 시 사라진다. 양측 절초 기세는 0/5에서 시작해 묶음 완료마다 1, 막기 성공·회피 성공·합 승리마다 해당 전투원이 1을 얻는다. 절초 3종은 기세 게이지 아래에 상시 표시하고 기세 5·연속 빈 수에서만 선택 가능하며, 예약은 즉시 기세 5를 소비하되 `[진행]` 전 점유 수 슬롯 클릭 또는 Enter로 전체 예약을 취소해 기세 5를 돌려받는다. 진행 뒤 중단·실패는 환불하지 않는다. 대표 시작과 최악 절초/밀착 Windows 성능 표본은 확보했다. `즉시 완료`는 진행 중 절초 타이머도 다음 프레임에 취소하고, Tab은 카드→절초 목록→수 슬롯→대상 타일→진행→재생/음향 제어 순서로 회귀 검증했다. 같은 조작 요소에 한국어 Godot 접근성 이름·설명도 부여했고, Windows UI Automation 노출과 실제 AudioStreamWAV 재생도 확인했다. 상세 규칙은 `docs/02_COMBAT_RULES.md`를 따른다.
...
# 전투 HUD·준비 수 정정 (2026-07-23)

- 절초 목록은 `강공`을 눌러 열지 않는다. 플레이어 절초 기세 바로 아래에 planning 중 상시 표시하고, 기세 정확히 5·연속 빈 수·입력 잠금이 버튼의 선택 가능 여부만 결정한다. Tab 순서는 카드 뒤 절초 3종을 거쳐 수 슬롯으로 이동한다.
- 2슬롯 행동은 첫 수 `[준비]`·마지막 수 `[실행]`, 3슬롯 행동은 앞 두 수 `[준비]`·마지막 수 `[실행]`이다. 준비 수는 비용·이동·피해 없이 `preparation` 확정 이벤트와 로그를 남겨 한 수씩 재생한다.
- 태세가 실제 실행되면 엔진의 `fortitude_next_attack`을 상태 패널이 읽는 `statuses`에도 동기화해 금빛 `[강건]` 태그와 설명을 표시한다. 속공 중단 보호를 소비하면 태그도 제거한다.
- MCP Godot 4.7 런타임에서 상시 목록·강공 미선택 기세 5 예약, 준비 이벤트, `[강건]` 상태 칩을 확인했다. 전체 standalone headless 회귀와 외부 사용자 플레이테스트는 이 소규모 HUD 정정 뒤 별도 `NOT_RUN`이다.
