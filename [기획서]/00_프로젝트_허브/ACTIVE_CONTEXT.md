...
## 최신 사용자 지시 · 프레임 시간축 전환 계획 · 2026-09-25

사용자는 3수·3수·4수 구조를 폐기하고, `https://claude.ai/artifact/Mc6edu6puFjP73ju4GrUBM`처럼 기술의 선딜·실행·후딜을 프레임 시간축에 배치하는 방향을 명시했다. 예시 게임의 실제 규칙·약공 배치·재생·다음 계획 복귀를 확인했다. 현재 제품은 아직 기존 구조이며, 전환 방향 승인과 구현 승인을 구분한다. 앞서 요청한 메인 캐릭터 제거·노드형 행로/능력치 사건·결과 복기·브리핑 개편과 함께 구체 구현/폐기안을 사용자에게 먼저 제시한다. 무공별 별도 모션은 제작하지 않는다. 현재 처리 중인 폐기 작업에 프레임 판정기·AI·저장 전환을 혼입하지 않는다.

구현 제안은 논리1칸0.1초·초기 계획 창30칸을 조정 가능한 시작값으로 두고, 기술 시작 시점/선딜/유효 판정/후딜과 경계 이월을 하나의 시간축에서 해결하는 방식이다. 상대의 비공개 계획은 노출하지 않고 관찰·전조·해결 이력만 사용하며, 준비 화면의 참조 구도와 삽화 선택은 유지한다. 기존3/3/4 계획 칸·묶음 판정·묶음 지속효과를 교체하고 같은 판정 이벤트로 연속 먹 연출과 하단 실제 결과를 표시한다. 수치·저장 이관·상세 구현 승인은 아직 대기다.
