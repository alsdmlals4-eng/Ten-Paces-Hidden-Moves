...
## 최신 승인 구현 · 10초 전투와 출사표 · 2026-09-25

사용자의 “지금걸로 맞춰서 만들자. 실제 게임데이터 구현,html 연결까지 진행해줘”에 따라 예시 검토에서 구현으로 승격했다. 책임 원본은 `docs/decisions/2026-09-25_FRAME_TIMELINE_AND_PROLOGUE.md`다. 새 게임은 출사표 → 시작 무공6중4/3성 삽화 선택 → 규칙과0.1초 배치 실습 → 첫 비전투 행로1회 → 브리핑 → 비무로 이어진다.

새 전투는10초/100틱 시간축에서 선딜·발동·후딜을 판정하며 비용·효과를 이월 중 중복 적용하지 않는다. 관찰 완료 후1/2/3단계의3/6/9초만 이미 확정된 적 계획을 공개한다. AI는 미확정 플레이어 계획/UI 의도를 읽지 않는다. 실제 판정 기록으로 연속 먹 연출·큰 하단 결과·시간별 복기를 표시한다. 무공별 별도 모션은 만들지 않는다. 기존3/3/4는 새 게임에서 제외되며 구형 저장의 명시적인 legacy 실행에만 남는다.

현재 구현은 격리 작업 폴더 `ink-preparation-layout-20260925`와 기존PR367에서 진행한다. 시간축 데이터/판정기, 새 native 준비·전투 UI, schema7 확정 저장/재개, 새 도입/사건 데이터와 HTML을 연결했다. 메인·출사표·준비용 글자 없는 배경3개는 실제 이미지 도구로 만들고 출처를 중앙 manifest에 등록했다. 시안7개와 실제 runtime 촬영은 서로 다른 근거로 표시한다. 새 파생 원화의 사용자 최종 시각 확정, 사람 플레이/학습/밸런스, Android와 출시 증거는 아직 NOT_RUN이다.

실행 결과: 코어152·도입158·획득64·저장 재시도32 등 새 회귀 PASS. Python 전체588검사에서 발견한2실패는 각각 자산 불변 기록의 새3ID 분리와 명시적인 legacy lifecycle fixture로 고쳤고 해당4검사 재실행 PASS. 실제 Windows에서 출사표부터 두10초구간/14.1초의 승리→보상→행로/사건과 별도 패배/복기를 촬영했다. HTML은15실제화면·473프레임/19.708초 영상,107검사와1357뷰/20667연결 PASS 및 브라우저 끝까지재생/미디어오류없음을 확인했다. 검토본 `build/windows/TenPaces-10Seconds-Preview-20260925.zip`은 실제 실행파일 시작·PCK·ZIP CRC/해시 검사를 통과했다. `실행하기.cmd`가 같은 폴더의 별도 save를 사용한다.

남은 경계: 기존 대력금강장/금강호체와 일부 합 기술의 정본 설명·실제 효과 차이를 임의 수치로 덮지 않았다. 세부 무공 정합화, 사람 플레이/밸런스, 새 파생 원화 최종 검수는 남아 있으므로 PR367 Draft 검토본이다. export 종료의 기존 ObjectDB45/resource22 정리 경고는 실제 실행 오류와 구분한다. 원격 exact-head 결과는 GitHub metadata, 로컬 상세 증거·남은 위험은 `docs/operations/2026-09-09_CLASH_DIRECTION_EXECUTION_REPORT.md` 최신 절을 따른다. 원본 사용자 작업·저장·코멘트·다른 Godot 세션은 보호한다.

## HTML 폐기 요청 처리 · 2026-09-25

공용 검토 revision108의102요청을 처리했다. 추적 이미지85개와 삭제대기 캡처2개는 실제 삭제,1개는 기존 삭제 확인, GUT13이미지는 최신 사용자 답변에 따라 HTML 목록에서만 제외하고 내부 파일을 유지, PR342 후보1개는 발행 사본을 삭제했다. 파생 미리보기와 연결된 sidecar를 포함한342파일 삭제를 확인했다. 이미지 번호·코멘트는 유지하고 일반 목록/본문/영상 poster의 끊어진 링크를 정리했다. 실제 카드에서 참조하던 구 배지·비용 atlas2개는 종류·비용 텍스트로 연결한 뒤 폐기했다. 전투 규칙·수치·준비 배치는 동일하다. `docs/blueprint/IMPLEMENTATION_READINESS.json`과 기존 실행 보고서의 최신 폐기 절이 처리/검증 근거다. PR367의 최종 시각 검수 대기 상태는 그대로다.

## 최신 준비 UI: 참조 그대로 · 2026-09-25

사용자의5676141a 참조와 “이미지 그대로 구현시키고 캐릭만 맞춰서 바꿔”를 적용했다. 이전 가로 배치 시안은 사용자 시각 불합격으로 대체한다.1086×1448 세로 산수/종이/먹 구도에 실제 UI를 얹고 현재 게임 인물만 교체했다. PR367 Draft에서 수정·검증·HTML·실행본을 갱신하며 생성 원화의 최종 시각 lock은 아직 미확정이다. 상세 근거는 기존 실행 보고서의 최신 참조 재현 절이다.

제품 코드ce56315d의 Windows 실행본은 `build/windows/TenPaces-Reference-Preparation-20260925-Windows.zip`이며 자동 제품 검사50/50·실패0과 압축 무결성을 확인했다. HTML 전투 준비는 실제1086×1448 캡처, 합 영상은708프레임/29.5초의 실제3/3/4 녹화다. 비무 제약 안내 경계와 실제 축소 UI 회귀를 교정했으며, 원격 작업 시간 제한으로 미실행됐던 후속23검사도 로컬에서 확인했다. 전달·집중 회귀·기존 실패와 최종 원격 검사 경계는 `docs/operations/2026-09-09_CLASH_DIRECTION_EXECUTION_REPORT.md`의 최신 전달 절과 PR367 live metadata를 따른다. 다음 사용자 검수는 참조 구도와 교체 캐릭터의 최종 모습이다.

## 준비 화면 UI 배치·구도 교정 · 2026-09-25

사용자의 최신 `UI배치,구도` 정정과960093f1참조를 기준으로, 기존 수묵 원화를 재사용해 소형 양끝 상태창·대각선 대치·중앙 거리·오른쪽 관찰·계획 한 줄/오른쪽 실행·5×2 삽화 카드/고정 상세를 구현했다. 규칙·AI·저장·합 연출의 계산은 유지한다. 실제 Godot AI4.2.3 입력으로 막기3개→실행→두 번째3수 준비 복귀를 확인했다. 신규4크기 기하·숨은 정보·관찰 갱신 검사와 기존 영향 검사를 연결했다. 최신 실행 보고서의 `참조 준비 화면의 UI 배치·구도 교정` 절과 GitHub exact-head가 검증·병합 상태를 소유한다. HTML의 전투 준비 화면도 실제 재촬영으로 갱신한다. 사용자 시각 검수와 Android는 별도다.

## 전투 실행 복구·삽화 선택·메뉴·대각선 준비 · 2026-09-25
PR365 정상 병합 main `9f0ce7ddf3a7aad95e82b725d99bbd5248bdd60d` / 검토 HEAD `92be01fb1385ab8373f06ad27117268acc538fd7` / 최신 원격33SUCCESS. 일회 승인 종료는 `docs/operations/2026-09-25_PR365_PROTECTED_CHANGE_APPROVAL_RECORD.md`. Windows 실행물은 `build/windows/TenPaces-Ink-Feedback-20260925-Windows.zip`, 최신 실제 화면11장·30.125초 영상은 HTML로 확인한다. 사용자 시각 검수는 별도다.

최신 사용자 5개 피드백과 Godot AI 업데이트 활용 승인으로 `549dc3aa`에서 격리 구현했다. 준비 행동의 강건 설명 저장 거부를 고쳐 실제 복사 저장의 이어하기와 다음 묶음 실행을 확인했다. 시작 무공은 삽화6종/선택4권, 메인은 새 여정·이어하기·도감·설정·종료, 준비는 가까운 왼쪽/먼 오른쪽 대치와 오른쪽 관찰·하단 카드다. 합의 앞뒤 위치·자세와 확대 컷을 연속 연결한다. 논리 전장·규칙·공개 경계·schema는 같다.

업데이트 Godot AI4.2.3의 실제 UI 입력·상태·캡처를 사용했다. 두 전체 검토와 결함 교정,11화면/30.125초 3/3/4 촬영, 실제 저장 보호·최종 검사·PR 전달은 `docs/operations/2026-09-09_CLASH_DIRECTION_EXECUTION_REPORT.md` 최신 절을 따른다. 구 일반 적 원화3파일 추가 삭제와 번호/코멘트 보존은 기존 폐기 ledger가 소유한다. 사람의 최종 연출 품질·모든 적의 개별 원화·Android 검수는 별도다.

## 수묵 화면 통일·하단 확대·기존 이미지 폐기 · 2026-09-25 추가 승인

PR363이 main `df3b7538f16e66208e29f4f4c04c85567c34ff5c`에 정상 병합됐다. 로컬602검사·Windows50/50·최신 원격34검사와 tracked tree 일치를 확인했다. 일회 승인 종료는 `docs/operations/2026-09-25_PR363_PROTECTED_CHANGE_APPROVAL_RECORD.md`를 따른다. 새 배경6장과 배외검객/도겸의 자세·초상 연결, 하단 결과 확대, 교체가 확인된 기존13이미지 실제 삭제가 범위다. 준비의 선택·배치·확정과 판정·AI·저장은 유지한다. 초상 경로만 바뀐 기존schema2/5/6 저장의 호환성을 검증했다. 무작위 첫 상대를 도겸으로 고정하지 않았으며 촬영만 seed1로 재현했다.

실제 화면9장과 최종28.583초/686프레임 녹화는 `docs/blueprint/evidence/ink-screens-20260925/manifest.json`에 소스 해시와 함께 연결한다. 전체 자동/Windows 빌드/브라우저/PR 및 두 차례 검토는 기존 실행 보고서의 최신 절을 따른다. 사용자 코멘트 원본과 원래45dirty 작업은 보존한다. 이번 새 이미지의 시각 품질·반복 관람 재미·Android 검수는 아직 별도다.
...
## 핵심 재미 두 축과 합 연출 재작업 · 2026-09-24

사용자는 핵심 재미를 **수싸움(행동설계)**과 **무협 액션을 보며 멋을 느끼는 연출(전투진행)**로 명시했다. 책임 원본은 `docs/decisions/2026-09-24_INK_WUXIA_STYLE_AND_FLOW.md`의 의도이며 두 경험을 판단→관람→다음 판단으로 연결한다. 1차 합 GIF는 멋이 부족하다는 사용자 피드백으로 `USER_REVISION_REQUIRED`다. 같은 8+3+9 방향 안에서 2차 원화·검객별9자세·먹 붓자국·6.4초 GIF/MP4 후보를 제작했고 `docs/visual-assets/candidates/TEN-INK-STYLES-20260924/clash-v2/README.md`에 실제 검수 결과를 누적한다. 자동 재생과 최종 모션 승인·사람의 재미를 구분한다. 실제 제품 경로와 사용자 코멘트는 그대로 보호한다.
...
이전 서버의120분 고정 종료가 늦게 불러온 이미지/영상 실패의 재현 원인이다. 새 주소의 Chrome/IAB에서 기존MP4가 재생됨을 확인했고 활성 페이지 확인으로 수명을 유지하도록 교정한다. 닫은 세션을 영구 준비 상태로 간주하지 않고 「블루프린트 열기.cmd」가 현재 발행본을 연결한다. 기존 미확인 원화·PR342 모션을 새 승인이나 현재main 구현으로 승격하지 않는다. 실제 사용자 리뷰revision3/3항목과 원래dirty45개 보호. 현행2회 전체검토와 추가표화 집중검토·브라우저 검증을 완료했다. 실제 사용자리뷰 해시 불변/제품diff0이다. 검증 세부/정상 PR #356 병합 현재 상태는 receipt.media_tables_followup/GitHub live에서 확인한다.

## 이미지141 삭제·142 교체·행동 삽화 통합 · 2026-09-23

사용자 후속교정은 HTML_MIGRATION_SPEC.md21절 및 기존 receipt.action_art_followup이다.141 옛 전투 캡처 삭제→실제 시작무공 SETUP 촬영 연결,142 이전파일 삭제→상태창 여백 편집 참고안 교체,실제 기초행동10종 삽화/텍스트5×2 통합. 옛 파일을 삭제대기나 과거 이미지 토글로 남기지 말라는 최신 지시를 따른다.142 번호와 코멘트 ID는 새 파일에 유지하고 편집 참고안을 게임 실행 증거로 표시하지 않는다. 구현·검수와 두 전체 검토를 완료했고 PR355로 동기화했다. 병합 여부와 최신 main은 receipt 통합 참조 및 GitHub live로 확인한다.
...
사용자가 구조도 상자와 화면 아틀라스를 합치는 구현안을 확인·승인했다. 첫 화면8개 상자에서 실제 아틀라스/무공 효과/상대·비무 제약/행동·전투/연출로 이어진다. 전체보기는112본문·16인·30기술·151자산·PM·누적 일지를 스크롤로 열람한다.
Godot4.7.1 현재 main 장면의 촬영 MP4 36개를 연결했다. 일부는 정적 표현 위주이며 PR342의 추가 모션 완료가 아니다. 실제 캠페인·사람 재미와 분리한다.
원본·재현·검증·집중 교정은 `docs/blueprint/HTML_MIGRATION_SPEC.md`15절과 기존 HTML receipt의 `interactive_atlas_followup`을 따른다. 월간 일지는 기존 JSON의9월23일 요약에 추가했다.
HTML 관련 현재 branch/PR/main은 GitHub live metadata로 재확인한다. 아래9월22일 신규 Godot 미실행 문장은 당시 전환 검증 기록이다. 이번에는 고정 상황의 실제 렌더 촬영만 추가됐으며 게임 제품 코드·PR342·외부 dirty 폴더는 변경하지 않았다.
...
product_stage: EVENT_CHECKS_HTML_IMPLEMENTED_VERIFIED
...
active_decision_state: EVENT_CHECKS_HTML_IMPLEMENTED_VERIFIED
