...
사용자 후속교정은 HTML_MIGRATION_SPEC.md21절 및 기존 receipt.action_art_followup이다.141 옛 전투 캡처 삭제→실제 시작무공 SETUP 촬영 연결,142 이전파일 삭제→상태창 여백 편집 참고안 교체,실제 기초행동10종 삽화/텍스트5×2 통합. 옛 파일을 삭제대기나 과거 이미지 토글로 남기지 말라는 최신 지시를 따른다.142 번호와 코멘트 ID는 새 파일에 유지하고 편집 참고안을 게임 실행 증거로 표시하지 않는다. 현재 검수/병합 상태는 receipt와 GitHub live를 확인한다.
