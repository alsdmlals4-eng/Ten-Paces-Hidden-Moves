...
- 절초 목록은 플레이어 절초 기세 아래에 두되 기본적으로 숨긴다. `강공` 선택으로만 열고, 기세 5·빈 연속 수·입력 잠금 상태가 선택 가능 여부를 결정한다.
...
절초 3종·피격 중단·강건·밀착·전투 불능 결과 상태는 구현 및 자동 검증 완료다. `[집중]`은 제거했다. 피해 중단은 그 수의 아직 실행 전 행동에만 적용하고 이후 수 계획과 같은 수 합은 유지한다. 양측 절초 기세는 0/5에서 시작해 묶음 완료마다 1, 막기 성공·회피 성공·합 승리마다 해당 전투원이 1을 얻는다. 기세 5가 되어도 목록은 자동으로 열지 않으며 `강공` 선택으로만 기세 아래의 절초 3종을 열고, 예약은 즉시 기세 5를 소비하되 `[진행]` 전 점유 수 슬롯 클릭 또는 Enter로 전체 예약을 취소해 기세 5를 돌려받는다. 진행 뒤 중단·실패는 환불하지 않는다. 대표 시작과 최악 절초/밀착 Windows 성능 표본은 확보했다. `즉시 완료`는 진행 중 절초 타이머도 다음 프레임에 취소하고, Tab은 카드→수 슬롯→대상 타일→진행→재생/음향 제어 순서로 회귀 검증했다. 같은 조작 요소에 한국어 Godot 접근성 이름·설명도 부여했고, Windows UI Automation 노출과 실제 AudioStreamWAV 재생도 확인했다. 상세 규칙은 `docs/02_COMBAT_RULES.md`를 따른다.
