...
주 책임 원본 하나를 갱신하고 실제 영향 소비자·테스트·파생본을 확인한다. 모든 문서를 관성적으로 수정하지 않는다.
...
| 행동 카드·비용·사거리 | `data/cards/basic_cards.json` 또는 02 | 03, 07, 08, 판정 엔진, UI | 데이터·표시·자원·경계 |
...
| UI·카드·접근성 | `docs/07_COMBAT_UI_SPEC.md` | 02, 08, 09, 10, 씬·렌더 | 정보 채널·포커스·해상도 |
...
| Base 채택·교훈 | `docs/11_BASE_ADOPTION_AND_LEARNING_LOG.md` | Base Rules, Audit, Learning Log | 제안·승인·검증 상태 |
...
| 책임 문서·경로·ID·Schema | Design Registry 또는 해당 정본 | Map, 소비자, Schema, tests, Workflow | reference-freshness |
| 발행 정책·생성기 | Design Registry | Schema, 생성기, Manifest, 파생본, 렌더 | source_only shape 또는 실행 가능한 발행 |
| Skill 추가·통합·mode | Skill Registry | SKILL.md, Base shared routes, Alias, Learning Log, tests | 무결성·stale ID |
| Work Mode·작업 계약 | AGENTS·AI Workflow | Registry, templates, Context, PR | execution-report |
| 승인 이미지·캡처 | Visual Source·Asset Manifest | 본책, 씬, 파생본 | 경로·사용자 검수 |
...
| 보류 재개 | 관련 본책 | 보류 인덱스, Roadmap, Test, Plan | 사용자 승인 |
| 파일 삭제·이동 | reconciliation 처리표 | 참조·파생본·복구·Git | 승인·reference-freshness |
...
- [ ] Design·Skill Registry·Legacy Alias·발행 영향을 확인했다.
- [ ] source_only 또는 실제 발행 파이프라인을 검증했다.
