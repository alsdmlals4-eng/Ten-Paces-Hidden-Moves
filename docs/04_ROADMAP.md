...
`TEN-DEC-20260908-DURABLE-RUN-CONTINUE-01`의 전달 순서는 strict 저장·전투 checkpoint → 실제 title/shell·생명주기 연결 → 격리된 새 프로세스 재개·10전 캠페인 → 저장 지연 검토 → 독립 검수·보호 전달이다. 현재 결과와 남은 Gate는 `docs/operations/2026-09-08_DURABLE_SAVE_EXECUTION_REPORT.md` 및 current owner에서 읽는다. 자동 재개 검증은 Human·Android·출시 검수를 대체하지 않는다.
