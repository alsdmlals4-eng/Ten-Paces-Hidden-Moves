...
- [x] 기존 기본 행동·공용 절초 3종 호환성.
- [x] RED→GREEN과 exact-head 자동 검증.

런타임 기반 구현은 최종 UI·AI·사람·밸런스 검증 완료를 뜻하지 않는다.
...
→ TenManualCombatResolutionEngine 어댑터 — 완료
→ exact-head PR Validation·Full Validation — 완료
```
...
| 위험 | 상태 | 다음 조치 |
|---|---|---|
| `RUNTIME_AUTHORITY_GAP` | `MITIGATED_RUNTIME_FOUNDATION` | UI·AI 채택 Gate |
...
- 10권 UI·AI 채택이 기존 기본 행동 회귀를 깨뜨림.
...
- `AMPLIFY`: 무공별 역할 차이와 실패 원인 설명.
- `CHANGE`: 사람 측정으로 확인된 수치만 별도 Decision으로 변경.
- `REMOVE`: 추가 입력, 숨은 계획 접근, 자동 합 승리, 능력치 쿼터.
- `DEFER`: 전체 UI·AI 채택, 최종 연출, 비스탯 노드 경제.
- `RETEST`: 자원 포화·기술 대체·관찰·등급 파밍 위험.
