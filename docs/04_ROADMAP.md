...
남은 개발의 요구·파일·인터페이스·저장 호환·인수 기준은 [남은 작업 설계 및 구현 명세](implementation/REMAINING_GAME_IMPLEMENTATION_SPEC.md)를 따른다. 시작4권과 회차 보유 전체의 분리 → 자유 수련 소비 → 플레이어 성장 연결을 먼저 다루며, 이미 구현된 저장·가변 편성·10전 흐름을 재구현하지 않는다. 이 명세의 후보 산식·경제·새 저장 버전은 별도 Decision 전 승인 규칙이 아니다.
