...
- 강건은 속공 피해에 대한 다음 1슬롯 공격 중단만 방지한다.
- 사망·강제 제어는 보호보다 우선한다.
...
`CombatResolutionEngine.resolve_bundle()`는 확정 state·logs·resolved actions·`presentation_events`를 한 번만 만든다. UI/VFX/오디오는 이를 재계산하거나 수정하지 않는다. 실제 피해는 이후 미실행 행동을 중단하고, 태세 실행이 준 `fortitude_next_attack`은 속공 피해의 다음 1슬롯 공격 중단만 막는다. `interruption_enabled`는 현재 true이며 Focus 상태는 없다.
