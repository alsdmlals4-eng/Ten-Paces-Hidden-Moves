...
> 구현 기준: PR #7 `147a031c75e96bff170d7f99016beb9e85b12066`
...
```text
combat_board_poc.json schema 16
...
`combat_resolution_preview.json`의 `enemy_plan_source`는 `public_state_ai`, `enemy_bundles`는 빈 객체다. 독립 회귀에서 AI를 끈 경우에만 fixture plan을 넣을 수 있다.
...
T0 플레이테스트 전에 필요 이상으로 선행 리팩터링하지 않는다.
