...
10칸과 비공개 3/3/4 계획 구조를 공용 엔진 계약으로 유지한다. 현재 T0 구현의 4/7 시작 좌표는 `IMPLEMENTED_LEGACY`; 최신 제품 정본의 공개 시작 거리는2이며 플레이어-facing 표기는 `거리 N`이다. `TEN-DEC-20260828-OPENING-DISTANCE-RUNTIME-MAPPING-01`은 시작 state·거리 계산·AI 입력·HUD·로그가 공개 거리2를 단일 의미로 공유하도록 확정한다. 내부 좌표 쌍은 implementation binding에서 경계·점유·회귀 검증과 함께 정하며, 두 상태를 같은 시작 규칙으로 취급하지 않는다.
