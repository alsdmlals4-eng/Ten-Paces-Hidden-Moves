...
- 현재 상대는 `CombatAiPlanner`의 공개 상태 기반 최소 AI다. JSON `enemy_bundles`는 AI 비활성화 fixture와 엔진 회귀에만 사용한다.
...
| 사용자 확인 | Windows STEP 0~10·TARGETING 10.5, HiGodot 최신 4/7·P1 runtime 확인 |
...
| `data/combat/combat_resolution_preview.json` | 판정 순서·회복량·방어 수치·AI 비활성화 시 재현 fixture |
...
- 기세는 양측 0/5에서 시작하고, 엔진이 묶음 완료·막기·회피·합 승리를 확정한 뒤에만 갱신한다.
...
- 같은 수의 유효 공격은 합으로 정산하며, 원공격력 차이 피해만 방어·회피 절차에 전달한다.
...
9. 묶음 완료는 양측 기세 1, 막기·회피 성공과 합 승리는 해당 전투원 기세 1 증가.
...
RESPONSE 10.6은 headless와 Windows Godot에서 기술 검증을 마쳤다. 실제 사용자 이해도 평가는 STEP 14의 별도 플레이테스트다.
...
- 실제 피해는 해당 수의 아직 실행하지 않은 행동만 기본 중단한다.
...
| 범위 | 상태 |
|---|---|
| 시작 위치 4/7 데이터·코드·테스트·SVG | 구현·정적·headless 확인 |
| 시작 위치 4/7 Windows | HiGodot runtime 확인 |
| STEP 0~10 | 구현·사용자 부분 확인 |
| TARGETING 10.5 | 구현·사용자 확인 |
| RESPONSE 10.6 | 구현·headless·HiGodot runtime 확인 |
| RESOURCE PREVIEW 10.6 | 구현·headless·HiGodot runtime 확인 |
| STEP 11 / Issue #11 | PASS — 자동·Windows 런타임·UI Automation·DEBUG 성능 증거 |
| STEP 12·13 | 구현·MCP 런타임 확인 |
| STEP 14 | 기계적 5시나리오 기록, 실제 사용자 관찰 `NOT_RUN` |
| typed domain model·event bus·save | 미구현 |
| 접근성·성능 | Windows UI Automation·DEBUG 성능 표본 PASSED; 실제 사용자·Release 목표 사양은 NOT_RUN |
## Issue #11 런타임 경계 갱신 (2026-07-23)

`CombatResolutionEngine.resolve_bundle()`는 확정 state·logs·resolved actions·`presentation_events`를 한 번만 만든다. UI/VFX/오디오는 이를 재계산하거나 수정하지 않는다. 실제 피해는 해당 수의 아직 실행되지 않은 행동만 중단하며, 이미 성립한 공격과 이후 수 계획을 되돌리지 않는다. 전투 불능은 이후 행동보다 우선한다. `interruption_enabled`는 현재 true이며 Focus 상태는 없다. 기세도 이 엔진에서 묶음 완료·막기·회피·합 승리로만 갱신한다.
