...
| Combat / Review | battle background remains largest mass | `frontal_courtyard_duel_background_01_v1`, player/enemy battlers, Dogyeom routing for `slot1_dogyeom` | HUD, timeline, cards, focus/selected/disabled state | range/target/resolve/review; `ultimate_ink_gold_sprite_sheet_rgba` | `USER_FINAL_LOCKED_RUNTIME_IMAGE + REUSE_PROJECT + GODOT_UI + SVG_VECTOR + SPRITE_SHEET` | P0 uses the user-final-locked 2026-08-31 frontal courtyard, shared-ground duel composition; runtime readback remains pending. Warm-dusk v2 remains planning-anchor-only and is never a runtime asset. |
...
|---|---|---|---|---|
| `SCREEN_COMBAT` / `OVERLAY_REVIEW` | `FRONTAL_COURTYARD_DUEL_BACKGROUND_01_v1` completed its separate brief and explicit final lock for `BattleBackground`; the warm-dusk planning anchor remains planning-only | `TEN-IMG-001`; `WARM_DUSK_TEN_STEP_COMBAT_ANCHOR_01_v2_NO_FLOOR_GRID` is planning-anchor-only; `FRONTAL_COURTYARD_DUEL_BACKGROUND_01_v1` is the distinct runtime background | focused Godot regression plus Godot 4.7.1 actual combat screenshot/node/log readback passed; Windows human comparison remains `NOT_RUN` | P1 |
...
|---|---|---|---|---|---|
| `COMBAT_BACKGROUND_01` | Combat, Review | `src/combat/battle_background.gd` | normal backdrop | `FRONTAL_COURTYARD_DUEL_BACKGROUND_01_v1` | `USER_FINAL_LOCKED_CANON_REGISTERED_IMPLEMENTED_MACHINE_RUNTIME_VERIFIED_20260831`; superseded ink-mist/twilight binaries removed from the current tree and recoverable through Git history |
