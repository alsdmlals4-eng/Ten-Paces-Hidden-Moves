...
| Combat / Review | top HUD, stage-only middle duel, lower planning/card surface | `frontal_courtyard_duel_background_02_v1`, reusable `frontal_courtyard_banner_overlay_01_v1`, v2 player/enemy battlers, Dogyeom routing for `slot1_dogyeom` | current-bundle timeline, cards, focus/selected/disabled state | range/target/resolve/review; `ultimate_ink_gold_sprite_sheet_rgba` | `USER_FINAL_LOCKED_RUNTIME_IMAGE + REUSE_PROJECT + GODOT_UI + SVG_VECTOR + SPRITE_SHEET` | P0 consumes the user-final-locked 2026-09-02 modular frontal courtyard pack through a separated top/middle/bottom screen grammar. The background/banner are clipped to the middle stage; the planner exposes only the current logical 3/3/4 bundle; v2 battlers share the foreground floor at a distant frontal scale. `TEN-RVC-20260902-003` records this exact combat surface. Warm-dusk v2 remains planning-anchor-only and is never a runtime asset. |
...
|---|---|---|---|---|
| `SCREEN_COMBAT` / `OVERLAY_REVIEW` | `FRONTAL_COURTYARD_MODULAR_ART_PACK_20260902_v2` completed user final lock: background, reusable banner overlay, and v2 generic battlers; `TEN-DEC-20260902-SCREEN-PARTITION-AND-DISTANT-FRONTAL-DUEL-01` adds the current distant stage composition | `TEN-IMG-001`; `WARM_DUSK_TEN_STEP_COMBAT_ANCHOR_01_v2_NO_FLOOR_GRID` is planning-anchor-only; `FRONTAL_COURTYARD_DUEL_BACKGROUND_02_v1` is the distinct current runtime background | focused Godot regression plus `TEN-RVC-20260902-003` actual partitioned/distant combat capture passed; Windows human comparison remains `NOT_RUN` | P1 |
...
|---|---|---|---|---|---|
| `COMBAT_BACKGROUND_AND_FOREGROUND_02` | Combat, Review, Main | `src/combat/battle_background.gd`; `src/ui/duel_foreground_banner.gd` | normal backdrop; mirrored left/right noninteractive banner pair | `FRONTAL_COURTYARD_DUEL_BACKGROUND_02_v1` + `FRONTAL_COURTYARD_BANNER_OVERLAY_01_v1` | `USER_FINAL_LOCKED_CANON_REGISTERED_IMPLEMENTED_MACHINE_RUNTIME_VERIFIED_20260902`; prior runtime background remains recoverable superseded evidence |
| `COMBAT_CHARACTER_BATTLERS` | Combat, Review, Main | `src/combat/combat_character_placeholder.gd`; `src/ui/main_title_screen.gd` | player, generic enemy, `slot1_dogyeom` | v2 generic player/enemy + existing Dogyeom | `USER_FINAL_LOCKED_CANON_REGISTERED_IMPLEMENTED_MACHINE_RUNTIME_VERIFIED_20260902`; remaining opponents require exact consumer + identity source |
...
| `CARD_ICON_ILLUSTRATION` | Combat | `src/ui/action_selection/action_choice_card.gd`, `data/cards/basic_cards.json`, `ActionViewModelAdapter` | source/category/cost/selected/disabled | basic atlas and final-locked martial/ultimate semantic atlas | all action sources retain one `ActionChoiceCard` hierarchy; `MARTIAL_AND_ULTIMATE_CARD_ILLUSTRATION_ATLAS_01` is `USER_FINAL_LOCKED_CANON_REGISTERED_IMPLEMENTED_MACHINE_RUNTIME_VERIFIED_20260831`, with semantic mapping at the view-model boundary |
