...
| `CARD_ICON_ILLUSTRATION` | Combat | `src/ui/action_selection/action_choice_card.gd`, `data/cards/basic_cards.json`, `ActionViewModelAdapter` | source/category/cost/selected/disabled | basic atlas active; martial/ultimate semantic atlas candidate | basic card illustrations are `COVERED_EXISTING`; `MARTIAL_AND_ULTIMATE_CARD_ILLUSTRATION_ATLAS_01` is `GENERATED_CANDIDATE_AWAITING_USER_FINAL_LOCK` and has no runtime consumer yet |
