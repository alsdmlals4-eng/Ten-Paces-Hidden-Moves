...
| Combat / Review | battle background remains largest mass | `frontal_courtyard_duel_background_02_v1`, reusable `frontal_courtyard_banner_overlay_01_v1`, v2 player/enemy battlers, Dogyeom routing for `slot1_dogyeom` | HUD, timeline, cards, focus/selected/disabled state | range/target/resolve/review; `ultimate_ink_gold_sprite_sheet_rgba` | `USER_FINAL_LOCKED_RUNTIME_IMAGE + REUSE_PROJECT + GODOT_UI + SVG_VECTOR + SPRITE_SHEET` | P0 consumes the user-final-locked 2026-09-02 modular frontal courtyard pack: shared ground, separate banner foreground, and v2 low-sword battlers. Exact new-art runtime readback remains pending. Warm-dusk v2 remains planning-anchor-only and is never a runtime asset. |
...
|---|---|---|---|---|
| `SCREEN_COMBAT` / `OVERLAY_REVIEW` | `FRONTAL_COURTYARD_MODULAR_ART_PACK_20260902_v2` completed user final lock: background, reusable banner overlay, and v2 generic battlers; the warm-dusk planning anchor remains planning-only | `TEN-IMG-001`; `WARM_DUSK_TEN_STEP_COMBAT_ANCHOR_01_v2_NO_FLOOR_GRID` is planning-anchor-only; `FRONTAL_COURTYARD_DUEL_BACKGROUND_02_v1` is the distinct current runtime background | focused Godot regression passed; exact new-art screenshot/node/log capture is required before runtime-verified status; Windows human comparison remains `NOT_RUN` | P1 |
...
|---|---|---|---|---|---|
| `COMBAT_BACKGROUND_AND_FOREGROUND_02` | Combat, Review, Main | `src/combat/battle_background.gd`; `src/ui/duel_foreground_banner.gd` | normal backdrop; mirrored left/right noninteractive banner pair | `FRONTAL_COURTYARD_DUEL_BACKGROUND_02_v1` + `FRONTAL_COURTYARD_BANNER_OVERLAY_01_v1` | `USER_FINAL_LOCKED_CANON_REGISTERED_IMPLEMENTED_PENDING_MACHINE_RUNTIME_VERIFICATION`; prior runtime background remains recoverable superseded evidence |
| `COMBAT_CHARACTER_BATTLERS` | Combat, Review, Main | `src/combat/combat_character_placeholder.gd`; `src/ui/main_title_screen.gd` | player, generic enemy, `slot1_dogyeom` | v2 generic player/enemy + existing Dogyeom | `USER_FINAL_LOCKED_CANON_REGISTERED_IMPLEMENTED_PENDING_MACHINE_RUNTIME_VERIFICATION`; remaining opponents require exact consumer + identity source |
