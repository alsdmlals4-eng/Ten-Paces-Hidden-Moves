...
- 작업 성격: 문서 정본 유지보수와 GrillMe 승인 누적
- 현재 GrillMe 승인 카운트: `3/10`
...
- Google Sheet 01·02·04·05·10·15·40·90·99
...
6. 최신 승인 기초 행동은 이동·보법·막기·회피·속공·강공·관찰·명상·준비·장풍의 10종이지만 현재 `basic_cards.json`과 ActionSelectionDock은 관찰·장풍이 없는 8종이다.
7. `태세`가 일부 최신 문서에 되살아났으나 2026-07-24 Decision에 따라 사용자 표시 정본은 `준비`다.
8. `랭크`가 전투 종료 S/A/B/C와 온라인 시즌 랭킹 사이에서 혼용될 위험이 있었다.
...
- 최신 승인 기초 행동 10종과 현재 구현 8종의 차이를 등록부에 명시.
- `태세`를 제거하고 독립 강화 행동 `준비`로 통일.
- `전조`를 강화 없는 다중 수 표시·점유 단계로 분리.
- 기초 장풍과 사거리 밖 합·절초기세·전투 종료 랭크 반영을 Decision/JSON으로 등록.
- 사거리 밖 합 승리의 위협 대응 가치를 사거리 안 합 승리와 동일하게 확정.
...
- 정확한 장풍 기본 피해·내공 계수·반올림.
- 사거리 밖 합 승리의 반복 계산·사건당 정확 점수.
- 정확한 랭킹전 관찰 변환표.
...
- 모든 활성 문서에서 `관찰 의존 ... 미결정`, `후보6명`, `공개 성향`, 신규 성장의 `공격력·방어력`, 사용자 표시 `태세` 표현 재검색.
- 기초 행동 10개 기획과 8개 레거시 데이터가 완료 상태로 혼동되지 않는지 확인.
- 전투 종료 S/A/B/C와 온라인 시즌 랭킹이 혼용되지 않는지 확인.
- planning JSON 파싱과 정적 계약 검사.
- exact-head Full Validation·PR Validation·Base v9 Adoption 확인.
- PR review thread와 head 이동 확인.
- 10/10 병합 뒤 main·Google Sheet 재조회.
