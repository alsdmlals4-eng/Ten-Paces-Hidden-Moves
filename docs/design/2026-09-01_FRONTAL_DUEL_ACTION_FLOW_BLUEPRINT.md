...
status: BLUEPRINT_READY_IMPLEMENTATION_PENDING
authority:
...
implementation_feasibility: PARTIAL
evidence_ceiling: BLUEPRINT_AND_CURRENT_SOURCE_READBACK_ONLY
```
...
| --- | --- | --- | --- | --- |
| BP-01 | `CombatBoardPreview`가 내부 `TileLayer`와 `FootAnchorGuide`를 만들고 `attack_direction`을 지원한다. | 플레이어에게 논리 전투판/공격 방향을 보이지 않게 해야 한다. | 논리 10칸은 resolver만 사용하고, 전투 화면은 공유 바닥·거리·발 접지로 읽힌다. | `CombatBoardPreview` presentation adapter |
| BP-02 | 기초 tray, 무공 dock, 절초 list/menu가 나뉘어 있다. | 기초·무공·절초가 모두 카드 형식으로 일관되어야 한다. | 하나의 `ActionChoiceCard` 계층에서 탭만 바꿔 5×2 이내로 비교한다. | `ActionSelectionDock` + panels |
| BP-03 | 카드 사실 정보는 renderer에 있으나 작은 surface에서는 밀도가 불안정하고 detail에 의존한다. | 카드에서 사거리·소모량·효과가 다시 보여야 한다. | 항상 이름·수·비용·짧은 효과를 보이고, 공격만 사거리를 같은 행에 표시한다. | `ActionChoiceCard` view-model |
| BP-04 | reveal overlay는 이미 timing별로 실행되나 “계획→공개→사건→정착”의 전체 상태가 가시적으로 분리되지 않았다. | 행동을 하나씩 공개하며 자연스럽게 겨루어야 한다. | 현재 수만 공개, 상호 카드 대조, 합/방어/회피/사거리 이유, 인물 motion, 최신 거리 순으로 이어진다. | `CombatActionRevealOverlay` + presentation adapter |
| BP-05 | 관찰은 잠긴 적 행동 유형만 공개하도록 실제 resolver가 지원한다. | 관찰이 상대 행동을 알려줘야 한다. | `관찰 기록 · [공격→대응]`은 planning층에 표시하되 기술명·대상·피해·뒤 수는 계속 숨긴다. | existing resolver output → status chip |
| BP-06 | 현재 CTA 문구가 runtime에서 `진행`인 경로가 남아 있다. | 행동계획 잠금과 실행 의미를 한눈에 구분해야 한다. | 선택 중에는 `행동계획 잠금`, 잠금 후 compact CTA는 `실행 · 3수`처럼 현재 묶음 수만 표시한다. | progress-button presentation only |
...
| `plan_locked` | 잠금된 현재 묶음과 compact `실행 · N수` | 실행, 잠금 해제(규칙이 허용할 때) | 미래 결과 미리보기 |
...
│                                                                                                     │
├─────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ 현재 계획 3수  [ 1 · 이동 / 전진 ] [ 2 · 전조 ] [ 3 · 공격 ]     [행동계획 잠금] → [실행 · 3수]  │
├─────────────────────────────────────────────────────────────────────────────────────────────────────┤
...
| progress | `src/ui/combat_progress_button.gd` | `행동계획 잠금` / `실행 · N수` copy와 compact layout | commit/resolution authority |
...
1. **계약 test RED** — 위 여섯 변경 전제와 current legacy carrier를 정확히 잡는다.
2. **공통 card grid 통합** — visual data only adapter로 기존 source panels를 재사용한다.
3. **전장 carrier 교체** — logical tile은 hidden state로, 이동만 front/back intent로 전환한다.
4. **reveal/impact refinement** — public resolved events로 card reveal → event VFX → distance settle을 연결한다.
5. **runtime capture** — PC 1440×900, 1280×800, reduced motion에서 current Godot scene을 기록한다.
6. **user review** — machine capture와 human/player evidence를 분리해 보여 준다.

```yaml
machine_verification: NOT_RUN_FOR_THIS_BLUEPRINT
runtime_visual_capture: NOT_RUN_FOR_THIS_BLUEPRINT
