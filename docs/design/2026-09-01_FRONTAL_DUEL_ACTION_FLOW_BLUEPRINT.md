...
### E. 이미지와 구조 설계의 이중 레이어

이미지 우선은 “이미지만 남기고 설계 정보를 지운다”는 뜻이 아니다. 파생 PDF는 매 화면의 **이미지 앵커**와 그 이미지만으로 읽기 어려운 **구조 설계 레이어**를 함께 보존한다. 이전 와이어프레임이 제공하던 흐름·입력·숨김 경계·상태 전이를 이미지 caption으로 축소하지 않는다.

| 레이어 | 반드시 담을 것 | 표현 규칙 | 증거 경계 |
| --- | --- | --- | --- |
| 이미지 앵커 | 실제 Godot 계획 화면, 승인된 석정/카드/합 시각 보드 | 출처 ID와 상태를 보이고, 사람이 현재 화면의 질량·구도·카드 삽화를 직접 확인하게 한다. | 실제 capture와 planning-only artifact를 혼동하지 않는다. |
| 플레이어 흐름 맵 | `계획 편집 → 행동계획 잠금 → N수 실행 → 현재 수 공개 → 합/정착 → 복기`, `3수 → 해결 → 3수 → 해결 → 4수` | 각 노드에 입력 또는 시스템 책임을 붙이고, “플레이어가 조작하는 것”과 “끝까지 숨길 것”을 함께 표시한다. | 미래 수·상대 미확정 세부·AI가 읽지 못하는 계획은 map에서도 노출하지 않는다. |
| 계획 구조 와이어프레임 | 상태 HUD, 공유 바닥/거리, 현재 계획 슬롯, compact CTA, 카드 grid와 상세 | 실제 화면을 대신하는 mockup이 아니라 **layout/content contract**로 표기하고, p3 실제 화면을 앵커로 연결한다. | 새 runtime screen 또는 새 asset이라고 주장하지 않는다. |
| 한 수 공개 구조 와이어프레임 | 현재 timing의 양측 카드, 원인 판정, 합/방어/회피, VFX·피격, 거리/자원 정착 | `현재 수만 공개`를 diagram 자체의 고정 문구로 두고, 실제 resolver event만 채운다는 경계를 표시한다. | plan-locked/reveal/impact capture가 없으면 `PENDING`은 그대로 남긴다. |

파생 PDF의 권장 순서는 `실제 화면/승인 시각 → 흐름 맵 → 계획 구조 → 카드 계약 → 공개 구조 → capture·handoff`다. 따라서 이미지와 도식 중 어느 하나가 다른 하나를 대체하지 않고, 구현자는 상태·입력·소비자를 추적하고 검수자는 실제 시각 방향을 대조할 수 있다.
