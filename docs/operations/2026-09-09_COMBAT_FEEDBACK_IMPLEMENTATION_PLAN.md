...
- [ ] **Step 4: behavioral GREEN.** All ten definitions and three legacy ultimates classify; opposite actor low mastery cannot borrow the other actor's ultimate. Assert actual reachable attack/recovery and generic response fallback visuals/placement. Canonical martial definitions with only outer response/response_combo and no execution facts get kind=outcome, band=-1, no attack motion, and 방어 준비 summary. Use explicitly synthetic pure-profile fixtures for currently unreachable self-evade+successful-counter and requirement-unmet combinations; do not report those as bundle execution. Test mixed blocked+hit, failed partial hit, interrupted/miss/block/evade priorities, missing texture fallback, reduced-motion/skip/mute state equality, deep-copy of forwarded martial facts, and durable resolved restore no event replay. Native test timeouts bound failures. Run focused tests then one full `python -m pytest -q`; run existing native ultimate/game-feel/inline/actor-binding relevant tests. Controller performs visible captures and independent final review.
