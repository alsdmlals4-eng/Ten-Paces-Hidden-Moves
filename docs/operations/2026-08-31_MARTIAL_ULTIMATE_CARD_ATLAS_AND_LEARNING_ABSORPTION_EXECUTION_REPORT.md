...
- **Godot:** exact Godot `4.7.1` headless editor parse exit `0`; `verify_action_card_source_unification.gd`, `verify_martial_action_panel.gd`, `verify_ultimate_action_panel.gd`, `verify_ten_manual_ui_ai_adoption.gd`, `verify_frontal_duel_assets.gd`, `verify_action_view_model_adapter.gd`, `verify_action_detail_panel.gd`, `verify_combat_action_selection_integration.gd` → 각각 `PASS`.
- **PR contract remediation:** PR #298의 첫 remote pass는 active semantic atlas를 누락한 combat asset contract와 historical no-illustration Godot assertion을 찾아냈다. 두 regression을 current final lock에 맞게 GREEN으로 갱신했고, one-time protected approval manifest와 `BUILD_APPROVAL_2026-08-31.md`를 same diff에 추가했다. 이어 full-validation workflow가 삭제된 `verify_combat_hypothesis.gd`를 실행하려는 stale CI reference도 발견하여, A2 retirement contract가 workflow reference까지 금지하도록 RED→GREEN으로 보강했다. 마지막으로 action-view-model adapter와 action-detail-panel 검사는 과거의 보이지 않는 기본 4권 loadout 가정을 제거하고, 빈 기본값과 명시적 해금 loadout fixture를 함께 검증하도록 고쳤다. 이 변경 전용 code head `3da6c777`의 PR #298 remote checks `29/29 SUCCESS`를 확인했다. 이 보고서 commit 이후의 mutable CI 상태는 GitHub live metadata를 owner로 유지한다.
- **증거 한계:** 위 결과는 automated + exact local runtime verification이다. 인간 플레이 비교, UX/human acceptance, 접근성 사용자, Android 실기기, release performance, merge/post-merge main readback은 여전히 `NOT_RUN`이다. PR #298 remote CI는 위 code head에서 `29/29 SUCCESS`였으나, 이 문서 갱신 commit의 최신 상태는 GitHub live metadata로 별도 확인해야 한다.
