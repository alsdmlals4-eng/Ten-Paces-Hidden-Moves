...
- 영구 스테이터스는 외공·근골·신법·내공·심안, 범위 1~15다.
- 초기 스테이터스와 기술 배수의 밸런스 기준점은 4다. 정확한 시작 총점·분배는 후속 결정이다.
- 관찰·이동·회피·준비의 기본 효과는 고정 전용이다.
- 그 외 연속 수치 효과는 고정 기본치와 최소 1개 능력치 참조를 가진다.

### 공격·합·연격

- 속공·강공은 고정 피해+외공 참조, 장풍은 고정 피해+내공 참조다.
- 장풍은 2수·내력1·사거리1~3이며 동일 조건 속공보다 피해가 낮다.
...
- 태그·고정치·능력치 배수·조건·비용은 별도 `price_id × quantity` ledger를 가진다.
