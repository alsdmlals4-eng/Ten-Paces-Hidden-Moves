...
- 현재 런타임은 관찰·장풍이 없는 8종으로 `IMPLEMENTED_LEGACY`다.
- `[전조]`는 실행 전 점유·표시 단계이며 강화가 아니다.
- `[준비]`는 독립 행동이며 다음 비이동 행동에 고정 강화 효과를 적용한다.
- 영구 스테이터스: 외공·근골·신법·내공·심안, 범위 1~15.
...
- 사거리 밖 합에서 양측 체력 피해가 0이고 두 공격이 유지되면 다음 순번 합도 진행한다.
...
전투 종료 `S/A/B/C`의 핵심 원자료는 다음 5개다.
...
- 태그·고정치·능력치 배수가 분리된 같은 데이터 구조로 두 번째 기술을 작성할 수 있다.
