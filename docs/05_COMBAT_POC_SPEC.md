...
> 주요 비무 1~5와 네 구간의 강호행로에서, 플레이어가 상대의 객관 정보와 `[관찰]`로 현재 묶음의 행동 종류를 좁힌 뒤 거리·합·방어·회피·중단과 성장 선택을 연결해 실제 계획을 바꾸는가?
...
- 대립 공격 효과의 **첫 피해 단위만 `[합]`에 참여**한다.
- 첫 합 패배·동점은 해당 공격 효과의 후속 피해 단위까지 취소한다.
- 첫 합 승리 공격의 후속 피해 단위는 순차 해결하지만 다시 합하지 않는다.
- 사거리 밖에서도 첫 합은 진행한다. 승자는 절초기세+1과 `ON_CLASH_WIN`을 얻지만 사거리·방향 실패 시 체력 피해와 적중 기반 효과는 0이다.
...
- 밀치기는 방어·합으로 체력 피해가 0이어도 적중이면 적용될 수 있으나 해당 피해 단위를 회피하면 적용되지 않는다.
...
- 정확한 가중치·정규화·등급 경계·파밍 방지는 후속 결정이다.
...
- 첫 피해 단위 합·후속 연격·방어·회피·중단의 결정적 이유를 설명한다.
