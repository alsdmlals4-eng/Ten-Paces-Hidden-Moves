...
핵심 재미는 수싸움(행동설계)과 무협 액션 관람(전투진행)이다. 확정한 수가 먹과 붓의 흐름으로 자연스럽게 이어지면서도 어떤 타격이 왜 살아남고 취소됐는지 읽을 수 있어야 한다. 모션 감소·음소거에서도 같은 결과를 읽는다.
...
2026-09-25 현재 consumer: 기존 행동설계 → **행동 실행** → 수묵 전투 진행(현재3/4칸을 한 줄로 유지) → 수별 실제 사건·하단 결과 → 묶음 전체 완료 → 기존 다음 행동설계/비무 종료. 승인 방향과 제품 적용은 `docs/decisions/2026-09-24_INK_WUXIA_STYLE_AND_FLOW.md`의 실제 게임 적용 절을 따른다. 아래의 카드 VS는 직전 consumer의 이력이다.
...
### 5.1 이전 정면 모션 consumer의 계약

이 절은 기존 준비 화면 및 legacy 비교/검증 consumer의 동작이다. 현재 확정 뒤 consumer는 승인된 수묵 자세의 발 앵커·검끝과 먹 텍스처를 사용하고, 앞 수의 회수 위치를 다음 수로 넘긴다. 검 대 검의 실제 합만 검 접촉으로 표시하며, 연출이 논리 타일 위치를 바꾸지 않는다.
