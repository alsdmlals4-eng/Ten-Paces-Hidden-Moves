...
7. 같은 수 동시 공격과 그 수의 미실행 행동 중단을 시각 순서 때문에 오해하지 않는다.
...
- 체력 0 뒤 추가 계획 입력을 잠그고 종료 결과 문구·로그를 표시한다.
- `결전 다시 시작`은 플레이어 4번·상대 7번, 30 체력, 초기 자원·기세·태세·예약·로그·연출 큐·오디오를 복원한다.
- 숨겨진 재시작 버튼은 일반 Tab 순서를 가로채지 않으며, 종료 때 표시되면 모션 감소 다음·음향 제어 이전의 포커스 대상이 된다.
...
- 절초 목록은 플레이어 기세 게이지 하단에 planning 중 상시 표시하며, 기세 5/5·연속 빈 수·현재 입력 잠금 여부를 버튼별 텍스트와 비활성 상태로 설명한다. 강공 선택은 이 목록의 조건이 아니다.
...
화면 상태는 `planning → committed → resolving → presenting_result → next_bundle_ready`로 기록되며, 체력 0이면 마지막 결과 뒤 `combat_ended`로 전환해 추가 계획 입력을 잠근다. 절초 목록은 기세 게이지 바로 아래에 planning 중 상시 표시하고, 기세 부족·연속 빈 수 부족·판정 중을 tooltip으로 별도 설명한다. 강공 선택은 절초 목록의 표시·선택 조건이 아니다. `planning`의 절초 점유 수 슬롯은 클릭 또는 Enter로 전체 예약을 취소하고 기세 5를 반환한다는 tooltip을 제공하며, `committed` 이후에는 입력 잠금과 함께 취소·환불도 막는다. 다중 슬롯 행동은 앞 점유 수마다 `preparation` 확정 이벤트와 `[준비]` 로그를, 마지막 수에는 실행 이벤트를 내보내 한 수씩 재생한다. 태세의 `[강건]`은 상태 패널 태그로 동기화하고 소비 시 즉시 제거한다. 결과 라벨은 60px 높이·한국어 단어 단위 줄바꿈으로 확정 이벤트의 중단·방향/사거리 실패·피해·전투 불능을 표시하고, 빠른 재생·즉시 완료·모션 감소·음소거·음량을 제공한다. 절차적 SFX는 묶음·막기·회피·합 승리의 기세 충전, 절초 예약, 검풍, 금속 충돌, 강타, 완전 막기, 회피, 중단, 전투 불능 역할음을 확정 snapshot과 이벤트에서 요청한다. `assets/vfx/ultimate_ink_gold_sprite_sheet_rgba.png`는 RGBA 알파 검수 뒤 활성화됐으며, 세 절초가 각 밴드를 AtlasTexture로 재생한다.
