...
기존 `07_COMBAT_UI_SPEC.md`가 UI 정보 배치와 카드 아트를 책임하고, 새 `10_COMBAT_PRESENTATION_PLAN.md`가 다음을 책임한다.
...
## 4. 이번 작업에서 Base로 승격한 내용

### 4.1 양방향 지식 순환

프로젝트가 Base에서 규칙을 받아오는 것과 프로젝트 교훈을 Base로 보내는 것을 별도 작업으로 보지 않고 한 작업 종료 흐름으로 연결한다.
...
- [ ] Base 버전 또는 기준 커밋을 기록할 프로젝트 파일이 필요한지 결정한다.
...
- 프로젝트 문서와 Base 사례에 공용·전용 경계를 명시했다.
