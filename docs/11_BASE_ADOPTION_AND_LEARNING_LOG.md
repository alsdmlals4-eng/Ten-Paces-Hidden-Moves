...
이번 프로젝트에서는 이를 복제하지 않고 `skills/TEN_PACES_PLANNING_HANDOFF_EXTENSION.md`에서 실제 경로와 검증으로 구체화했다.
...
- [x] Base 사례 두 건을 작성하고 사례 인덱스에 연결했다.
...
- [ ] Base 기준 커밋이 변경되면 프로젝트 문서와 스킬 확장의 차이를 재검토한다.
