...
**Architecture:** 구현을 런타임 전투 기반, 캠페인·성장, 표현·에셋의 세 독립 workstream으로 분리한다. planning JSON은 런타임이 직접 읽지 않고 명시적 adapter를 거쳐 `data/runtime/` 계약으로 변환한다. 각 workstream은 별도 RED/GREEN과 리뷰를 통과한 뒤 통합 branch에서 결합한다.
...
- planning JSON은 source-only이며 runtime에서 직접 읽지 않는다.
...
- Create branch: `codex/p0-poc-runtime-foundation`

**Interfaces:**
- Consumes: PR #45 head after `검수 완료` transition.
- Produces: isolated implementation branch with no uncommitted planning edits.
...
- [ ] Request code review after adapter/RunState contracts, before combat resolver replacement.
...
- [ ] Start only after runtime adapter and `RunStateStore` interfaces are merged into the implementation branch.
...
- Create: `docs/decisions/<date>_P0_IMPLEMENTATION_EVIDENCE.md`
