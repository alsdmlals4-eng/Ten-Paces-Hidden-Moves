...
current_binding_decision: TEN-DEC-20260828-REPOSITORY-ONLY-CANONICAL-WORKSPACE-01
...
notion_image_delivery_policy: RETIRED_MIGRATION_HISTORY_ONLY
notion_image_readback_policy: NOT_REQUIRED_FOR_CURRENT_EXECUTION
...
이 문서는 Base 공용 절차의 복사본이 아니다. 십보강호에서 새 채팅·새 담당자가 **현재 GitHub + repository owner만 fresh-read하여 동일한 실행 경계**를 복원하도록 프로젝트 고유 불변식과 최신 계약 locator만 유지한다. `TEN-DEC-20260828-REPOSITORY-ONLY-CANONICAL-WORKSPACE-01`은 r5.4의 Notion workspace 조항을 대체하며, 나머지 제품 안전 경계는 유지한다.
...
**GitHub repository**는 Markdown/JSON human-facing 및 structured canon, game data, code, Scene/Resource, tracked asset, tests, CI, runtime evidence를 소유한다.

승인된 의미 변경은 같은 Decision/Requirement identity로 repository human-facing/structured owner에 반영하고 destination readback한다. Notion은 신규 output·sync·attachment 대상이 아니며 historical migration input으로만 보존한다.
...
- `WHOLE_PROJECT_AUDIT_FIRST`: 새 프로젝트/새 채팅의 첫 material 작업, major gate·구조 변경·전수 검토·major closeout에서는 단일 파일 수정 전에 GitHub/repository owners/open workstream/실제 구현·검증·보호 범위를 함께 감사한다.
- `BOUNDED_DECISION_BATCH_AND_EARLY_CANON_SYNC`: 승인 결정을 과도하게 쌓지 않고 current Base intake owner가 정한 bounded 단위로 repository structured + human-facing canon을 조기 동기화한다.
- `PLAYABLE_SLICE_BOUNDARY`: 진행률은 문서/commit 수가 아니라 플레이 의미가 있는 작은 Slice로 측정한다.
- `CANONICAL_REFLECTION_AFTER_PLAY`: play/test 결과로 실제 finding이 생기면 교정 후 repository structured/human-facing truth를 readback한다.
...
Visual 제작 전 repository Visual Bible/asset record/requirement/inventory를 읽고 `VISUAL_REQUIREMENT_DELETE_TEST`, asset coverage, Art Style Lock을 확인한다.
...
→ 승인 뒤 repository asset lifecycle 및 destination readback
```
...
- repository Visual Bible / asset provenance / Flow owner
