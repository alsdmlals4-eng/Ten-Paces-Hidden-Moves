...
패배 분기는 숨은 정답이나 미래 행동을 보여 주지 않는다. 첫 5전의 목적은 돈을 잃게 하는 것이 아니라, 플레이어가 자신의 3슬롯 계획을 한 번 수정해 볼 수 있게 하는 것이다. `[영구재화]` 1/2/3 유료 재도전은 이 흐름의 후속 확장이다.
...
```mermaid
flowchart TD
    A["Main"] --> B["새 여정 · 기존 진행이 있으면 교체 확인"]
    A --> P["이어하기 · 마지막으로 확정된 준비 / 비무 / 행로 / 결과 / 완주 화면"]
    B --> C["시작 무공 6중4 + 시작 설정"]
    C --> D["짧은 강호 비무행 도입"]
    D --> E["Duel Briefing"]
    E --> F["Combat · 3/3/4"]
...
제목 화면의 **이어하기**는 이 컴퓨터에 남은 마지막 확정 진행을 엽니다. 비무 준비, 행로 선택, 보상 선택과 확정, 패배 뒤 남은 한 번의 재도전, 완주 요약이 함께 보존됩니다. 새 여정으로 바꾸기 전에는 기존 진행을 바꿀지 한 번 확인합니다.

행동을 실행하기 전에 고르던 미확정 배치는 다시 선택합니다. 이미 실행한 행동 묶음은 저장된 상대 행동과 결과로 이어집니다. 연출 도중 프로그램을 닫았다면 다시 열 때 그 묶음의 정착된 결과부터 계속합니다. 휴식을 선택한 뒤 다시 열어도 회복이 두 번 적용되지 않고, 보상을 확정한 뒤 다시 열어도 수련을 다시 받지 않습니다. 이어하기는 재도전 횟수를 늘리거나 다른 상대를 뽑는 기능이 아닙니다.

저장을 확인하지 못하면 추가 진행을 멈추고 **저장 다시 시도**를 표시합니다. 백업에서 복구했으면 그 사실을 알립니다. 읽을 수 없거나 이 버전과 맞지 않는 기록으로 새 여정을 시작할 때는 원본의 진단 사본을 보존하며, 사본을 보존하지 못하면 교체도 중단합니다. 창이 비활성화되거나 일시 정지되면 진행과 연출을 멈춥니다.

기술 계약과 현재 검증 범위: [저장·이어하기 결정](decisions/2026-09-08_DURABLE_RUN_CONTINUE.md), [실행 증거](operations/2026-09-08_DURABLE_SAVE_EXECUTION_REPORT.md). 사람용 기존 PDF는 발행 당시 기록으로 보존하며 이 구현으로 재생성하지 않습니다. 실제 Android 기기와 사람 플레이 검증은 아직 수행하지 않았습니다.
