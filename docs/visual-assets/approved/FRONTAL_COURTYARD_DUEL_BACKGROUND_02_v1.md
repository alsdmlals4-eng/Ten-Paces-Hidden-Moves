...
- **SHA-256:** `1f36bb1b853cf6008ca9d54bf71ebe1ce3916ce90db75a8caca1e62e5ad076fd`
- **Dimensions / alpha:** `1672×941`, RGB environment-only raster; transparency is not required.
- **Runtime consumers:** `BattleBackground` in `src/combat/battle_background.gd`; `MainTitleScreen` in `src/ui/main_title_screen.gd`.
- **Replaces:** active runtime use of `frontal_courtyard_duel_background_01_v1`; the prior approved bytes remain in Git and the manifest as `SUPERSEDED_BY_USER_FINAL_LOCK_20260902`.

## User approval and visual boundary

The user explicitly approved this background in the 2026-09-02 message: “배경,깃발도 방금 만든거 2개 승인”. It is an environment-only module for the frontal shared-ground duel: warm dusk stone floor, centered gate, distant mountains, and readable centre space. It intentionally contains no fighters, banners, weapon, text, UI, watermark, or copied reference material.
