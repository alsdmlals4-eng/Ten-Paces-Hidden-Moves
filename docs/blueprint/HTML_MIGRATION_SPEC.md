...
## 22. 화면 묶음·무공 공용 성장표·미디어 연결 수명 교정 (2026-09-24)

사용자의7항목 교정 지시는 같은 HTML 검수 계약의 REUSED_APPROVAL이다. 의도는 한눈에 화면과 기술을 비교하고, 실제 사용하는 이미지·카드 연출·효과·의견을 같은 항목에서 확인하는 것이다. 중간 크기의 이미지 목록 칸에 그림/종류/고정 번호/용도/코멘트를 함께 배치한다. 종류는 전투(준비·합·결과)와 강호행로(선택·주막/휴식)를 명시한다. 번호/코멘트ID는 바꾸지 않는다.

무공 성장의 공통10성 표는 한 번만 표시한다. 각 무공은 문파/주·보조 능력치와3성·7성·절초의 삽화, 비용/거리/순서,5·9성 추가 효과, 정확한 기술 촬영 링크를 표로 묶는다. 전역 연출 목록은 중간 크기 포스터와 재생 진입으로, 선택한 연출은 실제 MP4로 표시한다. 행로 사건 표는 사건 설명을 합친 셀과18개 선택지/조건/효과를 유지한다. 기존112쪽 설명과 승인 당시 자료는 유지한다.

확인한 원인: 이전 미리보기는120분 고정 종료되어 열린 HTML의 뒤늦은 이미지/영상 요청이 connection refused였다. 새 서버에서 같은 PNG와MP4 Range 요청206, 실제 Chrome/IAB MP4 재생을 확인했다. GIF 미지원으로 진단하지 않는다. 활성 페이지의 세션 확인으로 수명을 연장하고, 서버 종료/발행본 변경과 영상 실패를 페이지 안에 표시한다. 오래된 포트·토큰을 영구주소로 만들지 않는다. 기본 삽화는 같은 게임 atlas 영역을 정상 img로 표시해 깨진 SVG 그림 아이콘 확대를 방지한다.

142는 reader006의 연속된 full-width 그림 사이에 이전 주막 캡션이 놓여 오연결처럼 보였다. reader012 주막 원본은 맞으며 이미지 목록에서 그림과 캡션을 한 칸으로 묶어 재발을 막는다.142 참고 편집안/기존 승인 원화/실제 Godot 촬영은 계속 구분한다. 기초 행동·무공·기연의 게임 규칙·수치·저장·제품 코드는 이번 변경에 포함하지 않는다.

기준main d4dc0c18a7970bee64be860772ae7aca34116f87 / branch codex/blueprint-media-tables-20260924. 원래checkout dirty45개, 사용자 공용코멘트revision3/3항목, PR342/199/200은 보호한다. CURRENT_SOURCE_RELEVANCE_CHECK=REUSED_EVIDENCE + MDN HTTP Range/Video codecs 공식 문서의 제한된 미디어 적합성 확인. 실제 소비 경로와 브라우저 재현에 근거해 FEASIBLE. 새 게임 벤치마크나 같은 승인 인터뷰를 반복하지 않는다.

회귀5개 RED(이미지 목록 분류/사건 그룹/공용 성장 분리/수명 연장/세션 상태) 후 최소 교정, 관련 자동 검사, 실제 외부Chrome/IAB 재생·표·배치·반응형 확인을 수행한다. 전체검토는 현행 lean 계약의2회만, 이후 발견사항 집중 교정이다. 실행/검토/병합은 기존 receipt.media_tables_followup에 누적하며 게임 전체 실행·사람 재미·Android·출시를 주장하지 않는다.


§22 사용자 후속 명확화: 1번 대상은 연결구조도 자체가 아니라 그림/번호/용도/코멘트가 세로 나열된 본문 이미지 목록이다. 해당 목록을 모으며 구조도 노드·연결 의미는 유지한다. "다른 것들도 비슷한 케이스를 표로 모아줘"에 따라 전체112본문의 반복 이미지 패턴을 확인했다. 회차 흐름7그림은 각 단계 뒤 비교 칸으로, 상대16명은 중간 크기 목록으로, 별호/조합/약점/대응8쪽은 인물별 비교표로, 전투 실행3그림은 이미지-표시정보 표로, 같은 삽화를 반복한 연격3단계는 그림1개+조건 표로 모은다. 같은 번호/직접 코멘트와 모든 원래 설명·효과·순서를 유지한다. 별도 전체 검토를 새로 시작하지 않고 이 후속 범위만 회귀·집중 검토한다.


§22 검증 종료:69관련 자동 검사, 최종11집중 회귀,1087표시/12733로컬 링크와 자동저장/초기동기화 검사를 통과했다.464개 이미지/영상(36MP4 포함) 실제 Range응답206/MIME오류0. Chrome 매화삼첩 정확한 연출 버튼→2.233초 종료, IAB 격돌→합승리2.4초 종료. 전체보기는 공용성장표1개/30기술, 목록에 미리 장착한 영상0개이며 선택한영상만 실제 재생한다.390px/960px 비교 목록 및 좁은 표 내부 가로스크롤을 확인했다. 옛 페이지에는 새 발행 안내와 같은 구역을 유지한 새 링크가 표시된다. 현재 작업의Chrome스크린샷 API지연은 PASS로 표시하지 않고 시각 배치는 IAB에서 확인했다.

전체검토1/2 작성자·2/2독립 완료, 기존 무공바로가기 도착 위치를 RED→GREEN 교정했다. 사용자 추가12쪽 표화의 집중검토에서는 공통 진행/결과가 상대 카드에 붙는 문제를 발견해 별도 공통정보 표로 교정했다.119개 설명·45이미지 사용처·번호·리뷰ID보존, 추가전체검토없음. 실제 사용자리뷰revision3/3항목 해시 불변, 제품경로diff0, 원래dirty45·관련없는PR342/199/200 불변. 병합·현재main은 기존receipt 통합참조/GitHub live를 읽는다.

## 23. 사건별 선택과 능력치 판정·HTML 읽기 비용 개선 (2026-09-24)

사용자는 모든 사건을 기연에 연결하지 않고, 각 사건의 제목/상황 설명/세 선택지와 결과를 한 묶음으로 보여 달라고 지시했다. 이어서 능력치 확률 판정과 HTML 전반의 조사·가독성·최적화 구현을 승인했다. Decision `TEN-DEC-20260924-EVENT-CHECKS-HTML-01`을 따른다. 이번 후속은 앞선§22의 표시 교정과 구별되는 승인된 게임 규칙 변경이다.

Godot 원본은10사건/30선택지를 소유한다. 관련 능력치와 난도에 따라 성공률10~90%를 계산하며, 일부 선택지만 성공 뒤 추가10%로 기연을 얻는다. 일반 사건4개, 안전 선택, 성공/실패의 효과를 구분한다. 새 규칙은schema6, 기존schema1/2/5는 당시 규칙을 보존한다. 현재 플레이어의 실제 능력치 각4를 읽으며 미병합PR342의 영구 성장을 이번 완료 사실로 섞지 않는다. HTML에서는 각 사건의 설명을 한 번만 표시하고 선택/능력치/성공/실패/희귀 보상의 작은 표를 이어 붙인다. 새Windows 실행 촬영과 이전v1 촬영의 차이를 표시한다.

공식 web.dev·MDN·W3C 자료와 실제 화면/원본 크기를 대조했다. 목록에는480/960px 이하WebP 미리보기를 쓰고 확대는 원본을 연다. 승인 해시·번호·댓글ID·전체112본문·깊은 링크를 보호한다. 코멘트 저장은 변경된 항목만 갱신하고 복구용 임시 기록은 즉시 남긴다. 불변 발행 데이터만 검색 캐시를 사용하고, 한글조합이 끝날 때 검색한다. 넘치는 표에만 키보드 초점과 영역 이름을 붙인다. 연속 읽기를 막는 페이지 분할이나 본문 생략은 하지 않는다.

같은 본문의 이미지 파일 참조량 비교에서 전체보기630,125,046B→최대제공미리보기167,437,851B, 도감575,592,298B→25,840,582B였다. 실제 초기 다운로드·FPS 측정이 아니며 HTML 메타데이터는약224KB 증가했다. 원본 이미지 해시 불일치0개를 확인했다. 최종 발행에서는 촬영/기록 추가로 수치가 조금 달라질 수 있다. 정확한 테스트·두 전체 검토·브라우저·병합 근거는 기존receipt.event_checks_readability_followup에 누적한다. 사람 재미/Android/사용자 접근성/출시 성능은 NOT_RUN이다.


## 24. HTML 작업 방식 스킬화 · 2026-09-24

사용자가 이번 HTML 작업을 1차 완성 후 Base·다른 프로젝트에 예시로 전달할 수 있도록 스킬화하라고 추가 승인했다. [재사용 꾸러미](reuse-kit/README.md)는 개인 스킬 building-reviewable-html-blueprints의 전달 스냅샷, 동작 예시, ZIP과 실제 십보강호 consumer 연결을 담는다. 현재 원본을 읽는 생성/발행 경계를 유지하며 다른 프로젝트의 분류·규칙·영구 저장을 별도 어댑터로 연결한다. 게임 원화·실제 사용자 의견을 예시에 복사하지 않는다. Base 공통 정책이나 다른 프로젝트에 대한 적용은 아직 수행하지 않았다. 실행·집중 교정 근거는 기존 receipt.reusable_html_skill_followup이다.
