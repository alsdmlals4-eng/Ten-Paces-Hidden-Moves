...
아래 표는 구형 시작 무공 6권의 전투 철학과 현재 10권으로 이어지는 역할 계보를 보존한다.
...
현재 T0 런타임에는 공용 절초 3종이 역사 PoC 데이터로 존재한다. 초기 10권의 고유 절초는 기획·해결 순서·계획 예산까지 승인됐지만 런타임에는 아직 구현되지 않았다.

- 기본 절초: 공용 전투 검증용.
- 10성 절초: 무공별 최종 경지이며 현재 기획·예산 승인, 런타임 미구현.
...
- 승인된 10권 이름·문파·능력치 조합을 사용하는가.
- 주·보조능력치 권수나 균등 분포를 강제하지 않는가.
- 기존 6권은 역사 alias로만 사용하는가.
- 기술1과 기술2의 역할 비대체를 보존하는가.
- 이동 뒤 사거리 재확인과 상태 생성 선행 순서를 보존하는가.
...
- 현행 repricing overlay의 기존 6권 기술2 예산과 역사 alias를 독립 재조회.
- 7성 `+10` 통합 예산과 신규 4권 계획 예산 재계산.
- 9성 `10 + floor(20%)` 재계산.
- 초기 10권의 문파·능력치 적합성·성급 완전성·해결 순서 검증.
- 능력치별 권수·균등 분포·쿼터가 비활성인지 검증.
- Active Context·Roadmap·Lifecycle과 현재 배치 9/10 상태 일치.

사람 검증:

- 기술1/기술2 선택률.
- 7성 해금 뒤 기술1 대체율.
- 주·보조능력치가 문파·무학과 어울린다고 느끼는 비율.
- 9성 한 문장 효과 이해율.
