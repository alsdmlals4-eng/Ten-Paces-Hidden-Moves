...
후속 Decision `TEN-DEC-20260802-STARTING-TECHNIQUE-SOFT-GUARANTEE-01`에서 다음 보호 정책을 승인했다.

- 최종 확정 전 예상 영구 능력치 표시
- 각 선택 무공의 3성 기술 활성·잠금 미리보기
- 부족한 주 능력치와 필요한 점수 표시
- 시작 설정 확정 전 자유 분배·무공 선택 재조정
- 네 기술을 모두 여는 최소 기준선 `[추천 배분]` 제공
- 일부 기술을 잠근 전문화 빌드는 명시적 경고 후 허용

정확한 화면 배치와 제품 구현은 별도 Build 승인 전 `NOT_STARTED`다.
...
- 모든 시작 무공 4개 조합에 네 기술 동시 활성 배분이 존재함
- 동시 활성은 강제가 아닌 소프트 보장임
- 최소 해금 기준선 추천과 잠금 사전 고지

아직 미확정:

- 7성 기술과 10성 절초의 정확한 요구치
- 보조 능력치 요구 여부
- 시작 설정 화면의 정확한 배치·조작 순서
...
7. 소프트 보장 계약과 잠금 사전 고지가 Build Gate에 포함됨.
...
soft_guarantee_decision: TEN-DEC-20260802-STARTING-TECHNIQUE-SOFT-GUARANTEE-01
all_four_activation_required: false
preconfirm_unlock_preview: APPROVED_PLANNING_NOT_IMPLEMENTED
