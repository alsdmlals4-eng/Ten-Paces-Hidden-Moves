...
- [ ] 대립 공격 효과의 **첫 피해 단위만 합에 참여**한다.
- [ ] 첫 합 패배·동점은 해당 공격 효과의 후속 피해 단위까지 취소한다.
- [ ] 첫 합 승리 뒤 후속 피해 단위는 순차 해결하지만 다시 합하지 않는다.
- [ ] 비교값은 방어·회피 전 원공격력이다.
- [ ] 합 승리 절초기세는 공격 행동당 전투원별 최대 +1이다.
- [ ] 사거리 밖에서도 첫 합은 진행한다.
- [ ] 사거리 밖 합 승자는 절초기세와 `ON_CLASH_WIN`을 얻지만 체력 피해·`ON_HIT`·`ON_HEALTH_DAMAGE`는 발생하지 않는다.
...
- [ ] 밀치기는 방어·합으로 피해 0이어도 적중이면 적용될 수 있다.
...
- [ ] 가중치·정규화·등급 경계·파밍 방지·패배 등급이 미확정으로 표시된다.
...
## 18. App Flow Shell·ActionSelectionDock
...
- [ ] 첫 피해 단위만 합에 참여한다는 최신 override가 모든 활성 문서에 일치한다.
