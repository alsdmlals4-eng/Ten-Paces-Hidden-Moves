...
- [ ] 대상 부재·예측 빗나감·사거리 실패가 적중 이벤트를 생성하지 않는다.

## 5. 라운드·계획 확정 · 현재 2026-09-04 계약

- [ ] 한 라운드가 `3수 → 3수 → 4수`로 분할된다.
- [ ] 현재 묶음만 편집하고 단일 `[행동 실행]` 뒤 즉시 전투·해결 애니메이션으로 전환하며, 실행한 묶음은 다시 편집하지 않는다. 기존 lock → `[N수 실행]`은 `IMPLEMENTED_LEGACY` 회귀 fixture로만 보존한다.
...
- [ ] 기초·무공·절초 선택은 같은 카드 격자를 사용하며, 기초만 승인 atlas를 소비한다.
- [ ] 무공·절초 선택 카드와 이동/공격 의도 카드에는 삽화 `TextureRect`가 없다.
- [ ] 가장 앞 유효 연속 수 자동 배치와 연결 블록 이동·제거가 동작한다.
- [ ] 이동만 `접근 N칸 / 후퇴 N칸` 의미 카드로 확정하고, 공격·반격·절초는 공개된 상대를 자동 대상으로 하며 1~10 타일·좌우 방향 선택기는 보이지 않는다.
- [ ] 의도 선택 중 논리 타일 레이어와 타일 keyboard focus가 숨겨지고, 확정 뒤 현재 묶음 target readiness가 참이 된다.
...
- [x] Legacy runtime: `N수 실행` CTA와 실행 중 계획 편집 불가. 이 검사는 현재 player-facing copy의 PASS가 아니다.
- [ ] Current route/CTA: `3 candidates × 4 picks / 0/4→4/4`와 단일 `행동 실행`, 별도 Review 없음, current-card-only interruption 회귀 (`NOT_RUN`).
