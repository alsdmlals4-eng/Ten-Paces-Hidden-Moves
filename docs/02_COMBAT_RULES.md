...
2026-09-25 사용자 승인 [FRAME-TIMELINE-PROLOGUE](decisions/2026-09-25_FRAME_TIMELINE_AND_PROLOGUE.md)가 시간·관찰·도입 흐름을 대체한다. 실제 시간값은 `data/combat/frame_timeline.json`, 판정은 `src/combat/frame_timeline_engine.gd`가 소유한다.

- 행동 설계와 전투 진행 두 단계. 10초/100틱에서 해금 기술을 배치한다.
- 각 기술은 선딜·발동·후딜을 점유하며 같은 행위자의 기술은 겹치지 않는다. 비용은 선딜 시작에 한 번, 효과는 해당 발동 구간에서 판정한다.
- 창을 넘는 기술은 남은 동작과 비용·효과 처리 상태를 이월한다. 다음 창 첫머리를 임의로 덮어쓸 수 없다.
- 합·방어·회피·중단은 실제 시간과 거리로 판정한다. 방어/회피는 기술의 유효 시간에만 작동한다.
- 관찰은 완료 시점부터 1/2/3단계별 3/6/9초를 공개한다. 적은 내 미확정 계획을 읽지 않고, 이미 관찰된 배치를 바꾸지 않는다.
- 연출·하단 수치·복기는 같은 판정 기록을 읽는다. 다시 보기는 체력·비용·보상을 다시 적용하지 않는다.
- schema7은 새 규칙을 구분한다. schema1/2/5/6 저장은 기존 해석을 유지한다.

아래 과거 3·3·4 묶음, 수 슬롯, 관찰점 문구는 구형 저장/판정 호환 자료다. 새 시간축에는 적용하지 않는다. 시간 외 공격 공식·무공 효과·거리·AI 공개 경계는 각 현행 데이터와 일치하는 부분을 재사용한다.
...
- 플레이어는 현재 묶음만 편집하며 단일 CTA **`행동 실행`**으로 확정한다. 현재 묶음의 슬롯 수(`3` 또는 `4`)는 계획 정보로 보이되 CTA 이름을 바꾸지 않는다.
- `행동 실행`은 유효한 현재 묶음을 commit하고 즉시 전투·해결 애니메이션으로 전환한다. 실행 뒤 해당 묶음의 제거·재배치·이동 의도 편집은 불가하다. 기존 `N수 실행` runtime 표기는 `IMPLEMENTED_LEGACY`이며 `TEN-DEC-20260904-THREE-BRANCH-FOUR-CHOICE-JIANGHU-AND-HUMAN-BLUEPRINT-01`이 플레이어-facing 명세를 우선한다.
