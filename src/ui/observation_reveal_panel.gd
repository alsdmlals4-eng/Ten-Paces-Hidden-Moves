...
func _layout() -> void:
    ...
    if bool(get_meta("reference_preparation", false)):
        _frame.hide()
        _title.text = "관찰 정보"
        _title.position = Vector2(13,6)
        _title.size = Vector2(178,36)
        _title.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
        _title.add_theme_font_size_override("font_size",24)
        _title.add_theme_color_override("font_shadow_color",Color.TRANSPARENT)
        _hint.position = Vector2(16,217)
        _hint.size = Vector2(174,60)
        _hint.text = "관찰로 단서 확인" if _revealed_types.is_empty() else "확인한 행동 유형"
        _hint.add_theme_font_size_override("font_size",17)
        _hint.add_theme_color_override("font_color",Color("595548"))
        _hint.add_theme_color_override("font_shadow_color",Color.TRANSPARENT)
        _hint.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
        for i in range(_rows.size()):
            _rows[i].position = Vector2(12,51+i*63)
            _rows[i].size = Vector2(178,56)
            _rows[i].text = _revealed_types[i] + " 행동 확인" if i < _revealed_types.size() else ("아직 확인된 단서 없음" if i == 0 else "추가 정보 필요")
            _rows[i].show()
            _rows[i].autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
            _rows[i].add_theme_font_size_override("font_size",20)
            _rows[i].add_theme_color_override("font_color",Color("292b22") if i < _revealed_types.size() else Color("77715f"))
