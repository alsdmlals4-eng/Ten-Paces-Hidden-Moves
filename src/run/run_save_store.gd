...
func _cache_context() -> Dictionary:
    return {
        ...
        "frame_content_identity": codec.content_identity_for_schema(CODEC.FRAME_SCHEMA_VERSION),
    }
...
func load_checkpoint() -> Dictionary:
    ...
    if pointer.status != "ABSENT":
        ...
        if int(resolved.payload.schema_version) not in [CODEC.VARIABLE_SCHEMA_VERSION, CODEC.GIYUN_SCHEMA_VERSION, CODEC.EVENT_SCHEMA_VERSION, CODEC.FRAME_SCHEMA_VERSION] or pointer.slot != "v2_" + CODEC.digest(resolved.payload): return CODEC.error("CORRUPT", "Active pointer target mismatch")
...
func _save_variable(save_id: String, checkpoint_id: String, run_state: Dictionary, combat_checkpoint: Dictionary, active: bool, replace: bool) -> Dictionary:
    ...
    if _pending.is_empty():
        if not previous.is_empty() and int(previous.schema_version) in [2, 5, 6, 7] and _matches_request(previous, save_id, checkpoint_id, run_state, combat_checkpoint, active):
