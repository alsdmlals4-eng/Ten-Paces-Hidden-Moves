...
Base 공용 `auditing-and-refining-ui-art`의 방법·패턴·Godot 계약을 사용하되, 프로젝트 고유 전투 UX 책임 원본은 `docs/UX_UI_SYSTEM.md`다.
...
| 프로젝트 Mode | Base Mode |
|---|---|
| `ui-contract`의 플레이어 경험 | `experience-contract` |
| 화면·카드·3/3/4 정보 구조 | `flow-and-information-architecture` |
| 공용 패턴 판정 | `pattern-selection` |
| 상태·컴포넌트·피드백 | `design-system-contract` |
| Godot 상태 소유와 Signal | `godot-ui-contract` |
| `accessibility-review` | `accessibility-gate` |
| 플레이테스트 과제·통과 기준 | `playtest-contract` |
| `runtime-review` | `runtime-ui-audit` |
...
- 적 의도·거리·3/3/4 계획·합·복기의 UX 계약을 갱신한다.
...
- 프로젝트 UX 계약: `docs/UX_UI_SYSTEM.md`.
...
- 적 의도와 확인되지 않은 정보의 경계.
- 양측 위치·거리·밀착.
- 현재 자원·기세·상태.
- 라운드·묶음·수와 3/3/4 계획 슬롯.
- 배치·준비·실행·중단.
- 이동 목적지·공격 방향·절초 예약.
- 실행 전 비용·위험·불확실성.
- 합·방어·회피·필중 결과와 그 원인.
...
- 모든 핵심 입력에 프로젝트가 선언한 포인터·키보드·게임패드 경로가 있다.
...
- 플레이어가 카드·전장·슬롯·예상·결과만 보고 한 묶음을 설명한다.
