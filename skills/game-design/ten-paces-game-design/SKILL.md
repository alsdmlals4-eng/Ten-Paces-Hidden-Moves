...
6. 구현 필수 값과 플레이테스트 조정 값을 분리한다.
7. PoC는 가장 위험한 가설만 검증하고 전체 Vertical Slice로 팽창시키지 않는다.
8. 외부 게임·리뷰는 공식 사실·자기보고·행동 근거·해석을 분리한다.
9. 결과는 `KEEP / AMPLIFY / CHANGE / REMOVE / DEFER / RETEST`로 기록한다.
10. 관련 본책·Roadmap·Test·Plan·실제 데이터를 동기화한다.
