...
검수 중 원본을 바꾸면 재생성하고 본 작업이 시작한 서버만 종료/재시작한다. 활성 페이지가 매분 세션을 확인해 읽는 동안 연결을 유지한다. 기본120분 동안 미디어 요청·활성 페이지 확인이 없으면 종료된다. 열린 페이지의 연결 끊김/새 발행 안내를 확인하고 주소·PID를 다음 세션의 상시 준비 상태로 재사용하지 않는다.
