...
## Final CI discovery correction

Added exact push paths for the native campaign probe and its inherited sequential policy source. Extended the existing product-gate trigger test to require both specifically in push scope. Focused RED exit1 before workflow correction; full existing eight unittest checks GREEN plus product contract PASS afterward. No native probe/product change and no redundant campaign rerun in this final wave.

Controller independent exact-head467cd136 run: 141470 ms, 10 wins/0 draws, 10 rewards, 36 routes, 387 activations, exit0. Successful summary independently read from `.godot/native-controller-exact-head.log`; controller reports recurrent two-object teardown warning in process output. The issue remains unresolved, not suppressed or assigned an unverified production cause. Full detail and source distinction added to canonical report. Ready for scoped CI review and integration.
