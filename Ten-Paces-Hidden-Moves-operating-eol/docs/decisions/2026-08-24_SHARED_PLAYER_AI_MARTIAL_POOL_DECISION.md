...
- 구조화 계약: `docs/planning-data/approved_20260824_shared_player_ai_martial_pool_contract.json`
...
현재 `data/cards/martial_manual_cards.json`에 등록된 10권 전체를 player-learnable shared martial pool로 분류한다. 이 분류는 runtime manifest를 변형하지 않고 별도 planning-data 계약에서 소유하며, 회귀 테스트가 그 계약의 10권 ID와 실제 runtime manifest의 10권 ID가 정확히 일치하는지 검사한다.

현재 첫 Vertical Slice 시작 선택지는 이 중 6권이지만, 나머지 4권도 향후 적절한 획득 경로를 통해 플레이어가 배울 수 있어야 한다. 이번 변경은 그 4권의 획득 콘텐츠가 이미 구현됐다고 주장하지 않는다.

## 구현 경계

이번 bounded correction은 다음만 변경한다.

- shared-pool 구조화 planning-data 계약.
- opponent catalog의 player-learnable membership 및 runtime-registry 일치 회귀 테스트.
- AI 공정성 정본/Notion 설명.

변경하지 않는다.

- `data/` runtime manifest와 무공서 파일.
- 기존 15명 상대 배정.
...
- 별도 구조화 계약의 `player_learnable_manual_ids`는 현재 runtime manifest의 10권 ID와 정확히 일치해야 한다.
...
- planning-data의 player-learnable manual 집합과 runtime manifest의 실제 manual 집합이 달라짐.
- opponent card ID가 같은 manual/mastery의 shared registry unlock에 존재하지 않음.
- AI 전용 manual/technique를 허용함.
- AI가 비공개 플레이어 계획을 읽음.
